# MCP Apps Through Relay Gateway

Relay can already act as a gateway to downstream MCP servers for tools. MCP
Apps support should preserve that same gateway property: add Relay once, and
UI-capable tools from downstream MCPs should keep working through the Relay
MCP server.

This note is about the Relay work, not the GitHub issue demo app.

## Goals

- Proxy downstream MCP Apps without degrading the direct MCP server experience.
- Make UI-capable tools discoverable through Relay's normal MCP surface.
...
- Keep tool invocation and resource loading consistent across direct MCP,
  Relay gateway MCP, and future generated UI.
...
Relay should preserve both when present. It should not assume all hosts have
converged on one metadata key or MIME type yet.

## Resource Gateway

Relay needs a generic resource surface in addition to the current tool
surface.

Suggested relay-side APIs:

```ts
relay.resources.list({ source?: string, cursor?: string })
relay.resources.templates.list({ source?: string, cursor?: string })
relay.resources.read({ uri: string })
```
...
Relay can prefix downstream resource URIs and strip that prefix when it
...
relay:   relay+mcp://{sourceId}/ui/app/show_github_issue.html
```
...
```txt
downstream: ui://app/show_github_issue.html
relay:   ui://relay/{sourceId}/app/show_github_issue.html
```
...
- every resource URI exposed by Relay must be globally unique inside the
  gateway MCP server
- Relay must maintain a reversible mapping back to `{ sourceId, uri }`
...
Relay probably needs a conservative response-rewrite layer for proxied app
resources:
...
read method, or import a sibling resource by URI. If Relay has exposed the
...
```txt
ChatGPT / host
  iframe for downstream app template served through Relay
```

Relay should own the MCP transport/resource proxying, not wrap every
downstream app in an Relay app shell. A shell iframe is appropriate for
Relay's own generated UI feature, but direct downstream MCP Apps should render
...
iframe runtime, not the agent model. Relay can influence it only through:

- the tool descriptor metadata it exposes
- the resource body it serves
- the resource URI mapping it applies
- any app shell it intentionally provides for Relay-owned generated UI
...
For Relay-owned generated UI, a shell can provide controlled APIs:
...
The open generative UI PR adds an Relay app shell that can render React
generated by the model and proxy tool calls back through Relay. That shell is
...
not need a bespoke resource loader that bypasses Relay's MCP resource proxy.
...
- Which URI prefix shape should Relay standardize on for proxied resources?
- Should Relay expose both original and rewritten URI in hidden metadata for
  debugging?
...
- Relay generative UI PR:
  https://github.com/gothamdev244/relay/pull/263

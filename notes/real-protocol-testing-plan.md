...
Relay's plugin tests should make realistic scenarios easy to write. For
...
- Do not move plugin-specific protocol servers into `@relay-sh/sdk`.
...
```txt
@relay-sh/sdk/testing
@relay-sh/plugin-openapi/testing
@relay-sh/plugin-mcp/testing
@relay-sh/plugin-graphql/testing
```

The plugin packages own the protocol details. Core SDK testing owns only boring
Relay fixtures such as memory adapters, memory secrets, auto-accept
...
`packages/core/sdk/src/testing.ts` already provides the base Relay test
...
right home for cross-plugin Relay fixtures, but it is not enough for
...
For `@relay-sh/sdk`, either keep `src/testing.ts` as the subpath entrypoint
...
```ts
layer(OpenApiTestLayers.itemsApi.pipe(Layer.provideMerge(SdkTestLayers.relay())))(
  "OpenAPI plugin",
  (it) => {
...
- `makeRelayTestLayer(options)`, if repeated `createRelay(makeTestConfig)`
  setup becomes noisy.
...
Everything else should use Effect HTTP, Relay client APIs, or an explicit
package-local boundary.

Lint plan:

- Add `relay/no-raw-fetch` under `scripts/oxlint-plugin-relay/rules`.
- Register it in `scripts/oxlint-plugin-relay.js` and `.oxlintrc.jsonc`.
...
- Should `@relay-sh/sdk/testing` be a new subpath around the existing
...
- `scripts/oxlint-plugin-relay.js`
- `.oxlintrc.jsonc`

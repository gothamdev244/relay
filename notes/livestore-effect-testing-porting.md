...
as a wholesale tooling model. Relay already has the stricter Effect lint
...
Relay should port the shape, not the exact implementation. LiveStore is on
Effect 3, while Relay is on Effect 4 beta. The local helper should be tiny
...
Several Relay tests already have manual sleeps, `Promise.race`, test-level
...
Relay already uses `layer(TestLayer)(...)` in some OpenAPI and cloud tests.
...
It becomes useful if Relay adds property tests for:
...
Relay already uses `oxfmt` and oxlint:
...
Adding Biome would split formatter ownership without solving an Relay
problem.

### devenv / genie / effect-utils repo generation

LiveStore's config generation and `devenv` task setup are substantial
infrastructure. Relay's Bun/Turbo setup is smaller and easier to reason
...
Relay's lint posture is already stronger and more domain-specific. Relay
...
LiveStore disables several things Relay intentionally cares about, including
...
Relay mostly imports from `effect` directly. Do not introduce a broad
`@relay-sh/.../effect` barrel just for symmetry. It would add another import

...
The URL context determines which relay scope stack the server builds. Reads
...
Cloud should construct relays from URL-resolved context:
...
Do not rely on `relay.scopes.at(-1)` as "the current write scope" once
...
- Build global or workspace relay scope stack from resolved context.

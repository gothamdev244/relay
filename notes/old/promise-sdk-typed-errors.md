...
`@relay-sh/sdk/promise` wraps every Effect-returning method in
...
```ts
const exit = await relay.openapi.addSpec({ ... })
...
- Pros: `@relay-sh/sdk/promise` consumers don't import `effect` at all,
  surface stays small.
...
- `packages/core/sdk/src/promise-relay.ts` (`promisifyDeep`,
  `createRelay`)

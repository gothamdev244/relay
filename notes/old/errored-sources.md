...
`relay.jsonc` and calls the plugin's `addSource` / `addSpec` for
...
The row never reaches the DB. `relay.jsonc` still has it, so next
...
Effect.catchAll((e) => {
  ...
  return relay.sources.recordError({ source, message }).pipe(Effect.asVoid);
});
```

`relay.sources.recordError` is a new core method that writes a
...
hover/click, offers "Retry" which calls `relay.sources.refresh(id)`.
...
- `relay.sources.recordError` on the core SDK.

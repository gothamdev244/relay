...
## Today: flat, one scope per relay
...
- **SDK** — `createRelay({ scope: Scope, adapter, blobs, plugins })`.
  One `Scope` per relay instance. `scopeAdapter(rootAdapter, {...},
...
- **Cloud** — the WorkOS organization is the outermost scope.
  `createScopedRelay(scopeId, scopeName)` in
  `apps/cloud/src/services/relay.ts` is called per request with
  `{ org.id, org.name }` pulled from the session. One scope per request.

- **Local** — `apps/local/src/server/relay.ts` derives a single scope
  id from the working directory (`${basename(cwd)}-${sha256(cwd)[0..8]}`).
  One scope per relay process.

Coverage for the invariant lives in two places: `packages/core/sdk/src/
relay.test.ts` (SDK-level, in-memory adapter, two relays sharing
...
(`createRelay`'s adapter wrap) — not every plugin or every storage
backend.
...
- **Local global + per-folder.** `~/.relay/global` holds the outer
  scope; the current folder's `relay.jsonc` holds the inner. Run
  `relay` in folder A and you see folder A's sources layered on the
...
- `Scope` becomes `ScopeStack { read: readonly Scope[]; write: Scope }`,
  or `createRelay` grows a new shape and the old `Scope` path becomes
  a 1-element convenience.
- `ctx.scope.id` in plugin code is an implicit "the scope" today. The
  three callers in `relay.ts` that set `SecretRef.scopeId` from
...
- `scopeAdapter` signature is already list-shaped. Change the caller,
  not the wrapper.
- Secret resolution (`relay.secrets.get`) grows a shadowing pass:
...
`createScopedRelay` takes `(scopeId, scopeName)` today with the
...
- Cloud builds a `ScopeStack` from those: `[user, workspace, org]` read,
  ...
  override).
- `createScopedRelay` becomes `createScopedRelay(scopes: ScopeStack)`
  or a similar shape.
...
- Outer scope: `~/.relay/global` (or a named profile).
- Inner scope: the current folder, same derivation as today.
- `relay.jsonc` sync writes to the inner (folder) scope by default;
...
- `packages/core/sdk/src/core-schema.ts` — `scope_id` on source, tool,
  definition, secret.
- `packages/core/sdk/src/relay.test.ts` — SDK-level tenancy tests.

...
the relay reaches into them via `core.create({ model: "tool", data })`
on a raw `DBAdapter`.
...
// Relay now calls core.putTools(...), core.removeSource(...), etc.
// No more adapter.create({ model: "tool", ... }) scattered through relay.ts.
```
...
- Every `core.create / findOne / findMany / upsert({ model: "X" })` call
  in `packages/core/sdk/src/relay.ts` becomes a named method on

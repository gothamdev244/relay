...
The SDK stays storage-typed. The HTTP edge (`@relay-sh/api`) is the
...
### Storage layer (`@relay-sh/storage-core`)
...
- `createRelay` requires no observability service. `R = never`.
...
- Relay public methods (`relay.tools.list()`,
  `relay.sources.refresh()`, etc.) also surface raw `StorageFailure`.
...
### HTTP edge (`@relay-sh/api/observability`)
...
- **Cloud Worker** (`apps/cloud/src/observability.ts`) — provides
  ...
  `capture(...)` translation. Service tags (`RelayService`,
  `McpExtensionService`, etc.) hold the raw SDK shapes; the cloud app
  just does `Layer.succeed(McpExtensionService, relay.mcp)` —
...
- SDK code importing `Sentry.captureException` or referencing
  ...
  `@relay-sh/api`. If the SDK imports it, the layering is wrong.

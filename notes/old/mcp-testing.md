...
1. **Relay-level approval.** `relay.ts` → `enforceApproval` runs before
...
invokes such an op, the relay's `enforceApproval` fires. (This is subtle
because `invoke.ts` itself doesn't call `elicit()` directly — the relay
wrapper does.)
...
6. Call `execute` with code that (a) calls `tools.relay.openapi.addSource({ spec,
...
`tools.approveapi.approve.approveThing({})`. The relay's tool row id is
...
- "The openapi plugin doesn't elicit." It does, indirectly, through the
  relay's `enforceApproval` wrapper. `invoke.ts` doesn't have an `elicit()`

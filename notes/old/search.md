...
1. `relay.tools.list()` — `SELECT * FROM tool WHERE scope_id = ?`.
...
One method. No `key` — there's only one search provider per relay,
nothing to route.
...
Call site: `relay.search.query(q)`. (Rename the inner method from
`search` to `query` so we don't get stuck with `relay.search.search(q)`
...
export interface RelayConfig<TPlugins> {
  // ...
  readonly search?: (deps: SearchDeps) => ToolSearchProvider;
}
```

Why `listTools` instead of the whole `Relay`: breaks the chicken-and-
egg (relay can't be fully constructed before its search provider is)

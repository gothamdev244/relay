...
### 2. @relay-sh/cli package (packages/core/cli/)

- Mirrors better-auth's CLI approach: load config → collect plugin schemas → generate drizzle TS
- `relay generate --config ./relay.config.ts --output ./src/services/relay-schema.ts`
...
- `relay.config.ts` defines plugins with stub credentials (only used for schema shape)
- `drizzle.config.ts` points at both `schema.ts` (cloud tables) and `relay-schema.ts` (relay tables)
- Fresh `drizzle/0000_initial.sql` with all tables (cloud + core + plugin + blob), indexes, FKs

### 4. apps/local wired up

- `relay.config.ts` defines plugins + dialect sqlite
- `drizzle.config.ts` for drizzle-kit generate/push
- Generated `relay-schema.ts` with all sqlite tables + blob table
...
### The plan: re-derive sources from relay.jsonc

The `@relay-sh/config` package already maintains `relay.jsonc` as a source of truth for source configurations. The `config-store.ts` decorator intercepts `putSource`/`removeSource` on each plugin and mirrors to this file. So for existing users, `relay.jsonc` already has everything.

### Local app migration

1. On first run, `migrate()` creates all tables from the initial migration
2. Add a **sync-from-config step** after relay creation:
   - `loadConfig(configPath)` reads `relay.jsonc`
   - For each source, call the plugin's add method (`relay.openapi.addSpec()`, `relay.mcp.addSource()`, etc.)

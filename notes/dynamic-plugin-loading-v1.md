...
>    per-request relay pattern needed the Tag to be satisfied at
...
> 2. `<RelayPluginsProvider>` was added in `@relay-sh/sdk/client`
...
>    `@relay-sh/sdk/client` and are populated as fields on
>    `defineClientPlugin({ sourcePlugin, secretProviderPlugin })` so
>    everything frontend-facing flows through `virtual:relay/plugins-client`.

## Context

Before this work, plugins were workspace deps statically imported in
`apps/local/src/server/relay.ts` inside a `createLocalPlugins()`
factory; the cloud app had its own equivalent in
`apps/cloud/src/services/relay.ts`. They register secret providers,
...
storage → extension → consumer (`relay[pluginId]`), scope-aware ctx, closure
...
- A plugin is a single npm package. `bun add @relay-sh/plugin-foo`,
  add it to `relay.config.ts`, restart, and it works.
...
- A plugin author can write the whole thing importing only from
  `@relay-sh/sdk` (and `@relay-sh/sdk/client` on the frontend). Effect
  imports are optional — for authors who want them, not required for those
  who don't.
- `relay.config.ts` is the single source of truth — same file consumed by
  the schema-gen CLI and the runtime.
- Type inference end-to-end stays intact (`relay.foo.method()` autocompletes).
...
`relay.config.ts` holds real imports of plugin factory functions. Type
inference flows naturally; no codegen step.

```ts
// apps/local/relay.config.ts (after)
...
```jsonc
// @relay-sh/plugin-foo/package.json
{
  "name": "@relay-sh/plugin-foo",
  "type": "module",
  "exports": {
...
`@relay-sh/sdk/client`. Can only be imported in the `./client` entry, so
...
- **SDK-only.** Pure programmatic. No `routes`, no `handlers`, no `./client`
  ...
  `relay.<id>.method()`. Vite plugin notices no `./client` and skips the
...
`@relay-sh/sdk` (server) and `@relay-sh/sdk/client` (frontend). The SDK
...
### 7. `relay.config.ts` is the single source of truth

Today the schema-gen CLI reads `relay.config.ts` but the runtime hardcodes
`createLocalPlugins()` in `apps/local/src/server/relay.ts:96`. Consolidate:
the runtime imports from `relay.config.ts` too. One list, two consumers.

### 8. Canary plugin: build new tiny one first, then migrate openapi

Validate the shape with a minimal `@relay-sh/plugin-example` (one extension
...
> {
   ...
   *  same `relay.config.ts` drives both server and client. Author
   ...
   *  middleware. Endpoints automatically appear in the relay OpenAPI
   ...
   *  runtime wants:
   *    - local: at boot via `Layer.succeed(extensionService)(relay[id])`
   *      inside `composePluginHandlers(plugins, relay)`
   *    - cloud: per request via `Effect.provideService(extensionService,
   *      requestRelay[id])` inside the auth middleware via
   *      `providePluginExtensions(plugins)(relay)` */
...
local fine — one relay at boot, one Layer. Cloud builds a fresh
relay per HTTP request (Cloudflare Workers + Hyperdrive needs
fresh DB connections per request), so the relay that satisfies the
...
```ts
// @relay-sh/plugin-foo/src/shared.ts
import { HttpApiEndpoint, HttpApiGroup, Schema } from "@relay-sh/sdk";
...
```ts
// @relay-sh/plugin-foo/src/server.ts
import {
...
export const fooPlugin = definePlugin((opts?: FooConfig) => ({
  id: "foo" as const,
  packageName: "@relay-sh/plugin-foo",
  storage: () => ({ /* ... */ }),
...
Lives in `@relay-sh/sdk/client`. Server bundles cannot import this module.
...
`RelayApiClient` in `packages/react/src/api/client.tsx:11`.
...
export const createPluginAtomClient = <G extends HttpApiGroup.Any>(
  ...
  return AtomHttpApi.Service<`Plugin_${string}Client`>()(`Plugin_${opts.pluginId}Client`, {
    ...
    baseUrl: `/_relay/plugins/${opts.pluginId}`,
  });
};
```
...
```tsx
// @relay-sh/plugin-foo/src/client.tsx
import {
  defineClientPlugin,
...
against `FooApi` — same checks the existing `RelayApiClient.query("tools",
...
`@relay-sh/api` (also re-exported from `@relay-sh/api/server`).
...
} from "@relay-sh/api/server"

// Local — single boot-time relay; satisfy Tags eagerly.
const FullApi = composePluginApi(plugins)
const SpecHandlers = composePluginHandlers(plugins, relay) // self bound
...
// Cloud — per-request relay; satisfy Tags inside an HttpRouter middleware.
const FullApi = composePluginApi(plugins)
...
const ExecutionStackMiddleware = HttpRouter.middleware<{
  provides: AuthContext | RelayService | ExecutionEngineService
    | <PluginExtensionService union>,
}>()(
  Effect.gen(function* () {
    return (httpEffect) =>
      Effect.gen(function* () {
        const relay = yield* makeExecutionStack(...)
        return yield* httpEffect.pipe(
          Effect.provideService(RelayService, relay),
          provideExt(relay),         // binds each plugin's Tag per request
        )
      })
  }),
)
...
### Loader: `relay.config.ts` → runtime + frontend bundle
...
```ts
// apps/local/src/server/relay.ts (after)
...
`relay.config.ts` is evaluated, so no async loader needed.

**Frontend** — Vite plugin reads the same config, resolves each plugin's
`./client` export, exposes a virtual module:

```ts
// packages/vite-plugin-relay/src/index.ts (pseudocode)
export default function relayVite(): Plugin {
  return {
    name: "relay-plugins",
    resolveId(id) {
      if (id === "virtual:relay/plugins-client") return "\0" + id;
    },
    async load(id) {
      if (id !== "\0virtual:relay/plugins-client") return;
...
Host app consumes via `<RelayPluginsProvider>` at the root, then
...
import { useSourcePlugins } from "@relay-sh/sdk/client";
...
```ts
// src/shared.ts — only @relay-sh/sdk imports, no raw effect imports
import { HttpApiEndpoint, HttpApiGroup, Schema } from "@relay-sh/sdk";
...
```ts
// src/server.ts
import { Context, definePlugin, Effect, HttpApi, HttpApiBuilder } from "@relay-sh/sdk";
...
export const examplePlugin = definePlugin(() => ({
  id: "example" as const,
  packageName: "@relay-sh/plugin-example",
  storage: () => ({ count: 0 }),
...
```tsx
// src/client.tsx
import { defineClientPlugin, createPluginAtomClient, useAtomSet } from "@relay-sh/sdk/client";
...
// package.json
{
  "name": "@relay-sh/plugin-example",
  "type": "module",
  "exports": {
    "./server": "./dist/server.js",
    "./client": "./dist/client.js",
  },
  "relay": { "id": "example", "version": "0.1.0" },
  "peerDependencies": {
    "@relay-sh/sdk": "workspace:*",
    "react": "catalog:",
  },
}
```
...
```ts
// apps/local/relay.config.ts
import { examplePlugin } from "@relay-sh/plugin-example/server";
...
// package.json
{
  "name": "@relay-sh/plugin-rate-limiter",
  "type": "module",
  "exports": { "./server": "./dist/server.js" },
  "relay": { "id": "rateLimiter", "version": "0.1.0" },
  "peerDependencies": { "@relay-sh/sdk": "workspace:*" },
}
```

```ts
// src/server.ts
import { definePlugin, defineSchema, Effect, Schema } from "@relay-sh/sdk";
...
```ts
const result = yield * relay.rateLimiter.check("user-123", 5);
...
- **Backend** composes it into `createRelay`, exposes `relay.rateLimiter.*`. ✅
...
- **CLI** uses `relay.rateLimiter.*` directly. ✅
...
1. ✅ **Consolidate config.** `apps/local/src/server/relay.ts` reads
   from `relay.config.ts`. `createLocalPlugins` deleted. The cloud
   app's equivalent (`createOrgPlugins` in `apps/cloud/src/services/relay.ts`)
   also gone — driven from `apps/cloud/relay.config.ts` via a
   `(deps) => plugins` factory shape.
2. ✅ **SDK re-exports.** `Context`, `Effect`, `Layer`, `Schema`,
   `Data`, `Option` plus `HttpApi*` re-exported from `@relay-sh/sdk`.
   `@relay-sh/sdk/client` mirrors with `Schema`, `HttpApi*`,
...
4. ✅ **`defineClientPlugin` + `createPluginAtomClient` + Vite plugin.**
   `@relay-sh/sdk/client` exports the factory and helper.
   `@relay-sh/vite-plugin` resolves each plugin's `./client` from
   `relay.config.ts` and exposes `virtual:relay/plugins-client`.
5. ✅ **`@relay-sh/plugin-example`.** End-to-end proof: shared
...
7. ✅ **`<RelayPluginsProvider>` + hooks.** Host wraps once at
...
- Client contract: `packages/core/sdk/src/client.ts`
  (`defineClientPlugin`, `RelayPluginsProvider`,
...
- Local config + wiring: `apps/local/relay.config.ts`,
  `apps/local/src/server/main.ts`, `apps/local/src/routes/__root.tsx`.
- Cloud config + wiring: `apps/cloud/relay.config.ts`,

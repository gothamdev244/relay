...
Relay core should stay small: tools, sources, secrets, permissions, and
...
@relay/artifacts-cloudflare
  provider plugin for Cloudflare Artifacts

@relay/artifacts-local-git
...
The protocol package is official, but it is not core. Use `@relay/artifacts`
...
`@relay/artifacts-local-git` should likely use `isomorphic-git`, not shell
...
createRelay({
  ...
  protocols: {
    "relay.artifacts.store": "artifacts-cloudflare",
  },
});
```
...
import type { PluginProtocolProvider } from "@relay/sdk";

export const ArtifactStoreProtocol = {
  key: "relay.artifacts.store",
  label: "Artifact Store",
} as const satisfies PluginProtocolProvider<"relay.artifacts.store">;
...
```ts
// @relay/artifacts-local-git
import { definePlugin } from "@relay/sdk";
import { ArtifactStoreProtocol, type ArtifactStore } from "@relay/artifacts";
...
```ts
// @relay/workflows
import { definePlugin, type PluginCtx } from "@relay/sdk";
import { ArtifactStoreProtocol, type ArtifactsProtocolCtx } from "@relay/artifacts";
...
it.effect("creates a code-first workflow artifact and indexes a visual graph", () =>
  Effect.gen(function* () {
    ...
    const created = yield* relay.workflows.create({
      name: "Support triage",
      files: [
        {
          type: "put",
    ...
    const source = yield* relay.artifacts.readFile(
...
- [ ] Export `ArtifactStoreProtocol` and `ArtifactsProtocolCtx` from
      `@relay/artifacts`.
- [ ] Create `@relay/artifacts-local-git` using `isomorphic-git`.
- [ ] Add local Git provider tests for create project, write files, read files,
      diff, and fork.
- [ ] Create `@relay/artifacts-cloudflare` provider against Cloudflare
      Artifacts.
...
- [ ] Create `@relay/workflows` feature plugin.
...
- [ ] Create `@relay/generative-ui` feature plugin after artifact protocol is
      stable.

...
- One `ScopedCache` per plugin instance, captured by the closure
  `runtimeRef`. Lifecycle is the relay (one relay per session DO,
  built in `apps/cloud/src/services/relay.ts#createScopedRelay`).
...
three sequential `relay.tools.invoke` calls — passes against current
code.
...
- `relay.storage.transaction` 1.02s (token persist)
...
- **First tool invocation in a fresh DO.** `init()` builds the relay

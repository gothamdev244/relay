...
Relay plugins need a manifest that can be validated before plugin code runs. The manifest is the stable contract between a plugin package, the Relay host, the installer, the capability grant system, and the local/cloud runtimes.
...
The manifest should describe immutable plugin versions and compatibility with Relay.
...
The manifest declares which Relay versions and runtime APIs the plugin expects.
...
The manifest describes the package enough for validation and loading, but Relay should not require a custom distribution system at the start.

The default distribution path should be existing package distribution: npm packages, GitHub/git sources, and local paths for development. A future Relay registry can layer on top of the same manifest rather than replacing it.
...
This lets Relay evolve the manifest format without guessing based on plugin version.
...
Relay should not reinvent plugin distribution before it needs to.

The install surface should support package sources rather than assuming one custom marketplace or registry. Initial sources should include npm packages, GitHub/git repositories, and local paths. A future Relay registry should be an additional package source, not a replacement for those.
...
The string identifies the package source. Examples might include `@relay/openapi`, `github:owner/repo`, `https://github.com/owner/repo.git`, or `./plugins/my-plugin`. Tuple options are installation configuration, not manifest data.
...
`relay` should probably be a semver range for supported Relay versions.
...
Relay has two different consumption modes that the manifest needs to support.

An SDK-only consumer embeds Relay directly in an application. They need the SDK plugin factory and any required storage schemas, but they do not necessarily need HTTP routes, API handlers, React pages, or UI descriptors.

An API and React host needs more than the SDK plugin. It may need SDK registration, API route groups, API handlers, service bindings from the SDK relay extension, and React UI descriptors.
...
Relay does not need a custom publishing flow immediately. npm, GitHub/git, and local paths can be the initial distribution mechanisms. After a package is resolved, Relay can still cache the resolved package or build output as an internal artifact for repeatable local/cloud loading.
...
```json
{
  ...
  "name": "OpenAPI",
  "version": "0.1.0",
  "description": "Create Relay sources and tools from OpenAPI specs.",
  "compatibility": {
    "relay": ">=0.1.0",
    "runtimeApi": "0.1",
...
Should the manifest be embedded in `package.json`, stored as `relay.plugin.json`, or both?
...
What exact source spec grammar should Relay support for npm, GitHub/git, registry, and local path plugins?

Should GitHub support mean git clone, GitHub tarball download, npm-style `github:owner/repo`, or all of them?

How should a future Relay registry compose with npm/GitHub distribution instead of replacing it?

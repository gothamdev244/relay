...
Relay's plugin system is the main extension mechanism for the product. It should let Relay add new capabilities without putting every capability into the core SDK, and it should let users, teams, vendors, and the community extend Relay without waiting on first-party product work.
...
Plugins should be a core part of Relay, not an optional add-on.
...
Plugins should work locally and in the cloud through the same product architecture. The runtime implementation can differ, but plugin authors and Relay product surfaces should not have to reason about separate local-only and cloud-only plugin systems.
...
Plugins should compose with Relay's existing primitives: tools, sources, scopes, secrets, definitions, connections, and execution.
...
This document does not define every extension point Relay will ever support.
...
Relay should separate plugin authoring from plugin execution.
...
Relay should support multiple plugin categories through one conceptual plugin system.
...
First-party plugins are maintained by Relay. They should provide common capabilities without expanding the core SDK unnecessarily.
...
The manifest should also include compatibility metadata, such as supported Relay versions. This avoids loading plugins against incompatible host APIs.
...
Plugin loading should be deterministic. Relay can resolve and prepare plugins in parallel, but activation should happen in a stable order so side effects, conflicts, and diagnostics are predictable.
...
Relay should track plugin provenance for registered capabilities. When a tool, source, secret adapter, route, or UI surface exists because of a plugin, the product should know which plugin and version provided it.

## Conflict Policy

Relay should define conflict behavior early.
...
For source and tool visibility, Relay should preserve the existing scope model: reads resolve through an ordered scope stack, and writes target an explicit scope. Plugin-provided tools and sources should participate in that model rather than bypassing it.
...
Scopes define ownership and visibility for Relay resources. Plugins need to integrate with scopes rather than inventing a separate tenancy model.
...
Plugin-created resources should write to explicit scopes. Plugin reads should resolve through the Relay scope stack only when the plugin has permission to read those scopes.
...
Relay should assume third-party plugin code is untrusted.
...
Cloud runtime state should not live inside the isolate. Isolates should be treated as cacheable but disposable. Durable state should live in host-managed storage such as D1, KV, R2, Durable Objects, or Relay's own storage adapters, always namespaced by plugin, version, tenant, and scope as appropriate.
...
Relay should support local development and package-based distribution.
...
Relay should support a safe mode or pure mode that disables external plugins while still allowing core and internal functionality to start.
...
Extension points should prefer explicit inputs and outputs over mutation of host objects. Where ordered mutation is needed, Relay should make ordering deterministic and visible.
...
Plugin errors should be isolated. A broken plugin should fail to load, fail its own operation, or be disabled without preventing Relay from starting when possible.

## Architecture Implications

Relay core should own the primitive contracts: tools, sources, scopes, secrets, plugin registration, capability grants, and execution bridge interfaces.

Relay core should not own every product capability. OpenAPI, GraphQL, MCP, custom tools, execution logs, workflow-related features, and generated UI support can be provided by plugins where practical.
...
emdash demonstrates the cloud-side pattern Relay likely wants: Dynamic Worker Loader, no ambient network, bridge-mediated access, serialized request boundaries, and durable state outside the isolate.
...
Pi demonstrates an ergonomic plugin authoring model: small typed API, registration during load, lifecycle events, package discovery, and composable extension points. It does not provide the security model Relay needs.

opencode demonstrates practical plugin loading and distribution: simple config shape, provenance tracking, compatibility checks, deterministic activation, install-time script suppression, safe mode, and lifecycle cleanup. Its plugins are trusted, so Relay should borrow ergonomics but not the trust assumption.

...
the CLI (`relay` npm package plus its platform packages) and the
`@relay-sh/*` library packages (`core`, `sdk`, and the public plugins).
...
5. The release workflow then does two things in parallel:
   - Publishes every `@relay-sh/*` library package whose current version
   ...
   - If `apps/cli/package.json` bumped, tags the commit and dispatches
     `.github/workflows/publish-relay-package.yml`, which:
...
To pack the `@relay-sh/*` library packages without publishing:
...
If your PR adds a `.changeset/*.md` for the `relay` package, also

...
const isDirectExecutorSchemaImport = (specifier) =>
  specifier === "./relay-schema" ||
  specifier === "./relay-schema.ts" ||
  specifier === "../services/relay-schema" ||
  specifier === "../services/relay-schema.ts" ||
  specifier.endsWith("/services/relay-schema") ||
  specifier.endsWith("/services/relay-schema.ts");
...
export default {
  meta: {
    type: "problem",
    docs: {
      description: "Disallow direct imports of cloud relay-schema outside sanctioned DB wiring.",
    },
  },
  create(context) {

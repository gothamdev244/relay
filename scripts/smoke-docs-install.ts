 ...
 * name (`@relay-sh/sdk`) instead of relying on workspace resolution.
 */
import { $ } from "bun";
...
const documentedPackages = ["@relay-sh/sdk", "@relay-sh/plugin-openapi"] as const;
...
const tmp = await mkdtemp(join(tmpdir(), "relay-docs-install-"));
...
try {
  ...
  await $`node --input-type=module --eval ${`const sdk = await import("@relay-sh/sdk"); const openapi = await import("@relay-sh/plugin-openapi"); if (typeof sdk.createRelay !== "function") throw new Error("missing createRelay"); if (typeof openapi.openApiPlugin !== "function") throw new Error("missing openApiPlugin");`}`.cwd(
    tmp,
  );
} finally {

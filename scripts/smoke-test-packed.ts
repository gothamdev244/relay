#!/usr/bin/env bun
/**
 * Pack-and-import smoke test for the public `@relay-sh/*` packages.
 *
 ...
 *      with npm `overrides` pointing every other `@relay-sh/*`
 ...
 *
 * Failures for `@relay-sh/*` package not-found are hard failures —
...
const PRIVATE_PACKAGE_RE = /Cannot find package '(@relay-js\/[^']+)'/;
...
const smokeTestPackage = async (
  ...
  const tmp = await mkdtemp(join(tmpdir(), "relay-smoke-"));
  try {
    // npm `overrides` forces transitive `@relay-sh/*` deps to resolve
    // to their local tarball instead of whatever's published on npm.
    // Without this, a plugin built against an unreleased symbol in
    // `@relay-sh/sdk` would silently install the published `sdk`

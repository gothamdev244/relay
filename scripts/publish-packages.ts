#!/usr/bin/env bun
/**
 * Publishes the public @relay-sh/* workspace packages to npm.
 *
 ...
 *
 * Workspace-only `@relay-sh/*` peer deps (e.g. `@relay-sh/api`,
 * `@relay-sh/react`) that aren't in `publishable` are stripped from
...
const applyWorkspaceVersions = async (
  ...
  const renameDepBlock = (block: DependencyBlock | undefined): DependencyBlock | undefined => {
    ...
    for (const [key, value] of Object.entries(block)) {
      if (publishable.has(key) && value.startsWith("workspace:")) {
        ...
        // Workspace-only `@relay-sh/*` regular dep that we don't
        // publish (e.g. `@relay-sh/api`). Strip it: it's not in the
   ...
   * publishable peers, but DROP non-publishable `@relay-sh/*` peers.
   * They reference workspace-only packages (`@relay-sh/api`,
   * `@relay-sh/react`) that don't exist on npm, so leaving them in
   * the packed manifest emits install-time warnings for unresolvable
   * packages. Non-`@relay-js` peers (`react`, `@tanstack/*`,
  ...
  const renamePeerDepBlock = (block: DependencyBlock | undefined): DependencyBlock | undefined => {
    ...
    for (const [key, value] of Object.entries(block)) {
      if (publishable.has(key)) {
        ...
        // Workspace-only `@relay-sh/*` peer that we don't publish —
   ...
   * `@relay-sh/*` peer we don't publish, mirroring `renamePeerDepBlock`

...
afterEach(() => {
  if (previousDataDir === undefined) {
    ...
    process.env.RELAY_DATA_DIR = previousDataDir;
  }
...
describe("daemon host and scope identity", () => {
  ...
  it("resolves relative scope directories against the current workspace", () => {
    const workspace = mkdtempSync(join(tmpdir(), "relay-daemon-scope-"));
    try {
      process.chdir(workspace);
      process.env.EXECUTOR_SCOPE_DIR = "relay.jsonc";

      expect(currentDaemonScopeId()).toBe(`scope:${join(workspace, "relay.jsonc")}`);
    } finally {
  ...
  it.effect("keeps daemon pointers distinct for scope ids with the same sanitized form", () =>
    Effect.gen(function* () {
      const dataDir = mkdtempSync(join(tmpdir(), "relay-daemon-state-"));

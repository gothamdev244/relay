...
/** Dist directory name (e.g. dist/relay-linux-x64). Only used as a build
 *  artifact convention; the actual npm package name inside is `executor`. */
const targetPackageName = (t: Target) => `relay-${platformTag(t)}`;
...
const binaryName = (t: Target) => (t.os === "win32" ? "relay.exe" : "relay");
 ...
 * at runtime. We copy the `.node` file next to the relay and main.ts sets
...
const buildBinaries = async (targets: Target[], mode: BuildMode) => {
  ...
  // to each target's relay. `bun install --frozen-lockfile --cpu=* --os=*`
  ...
  try {
    for (const target of targets) {
      ...
      // Copy @napi-rs/keyring native binding next to relay — bun --compile
      ...
      // The local alias name (`relay-linux-x64`, ...) is what the
      // wrapper's launcher passes to require.resolve at runtime. It's
      // bound at install time by npm's `npm:relay@<variant-version>`
...
const buildWrapperPackage = async (binaries: Record<string, string>) => {
  ...
  await writeFile(join(binDir, "relay"), NODE_SHIM);
  await chmod(join(binDir, "relay"), 0o755);
    ...
    JSON.stringify(
      {
        ...
        //   "relay-linux-x64": "npm:relay@1.4.14-linux-x64"
        ...
        // `node_modules/relay-<plat>-<arch>/` so resolution just works.
...
const publish = async (channel: string) => {
  ...
  for (const entry of new Bun.Glob("relay-*/package.json").scanSync({ cwd: distDir })) {
...
const createReleaseAssets = async () => {
  // The dir name (e.g. "relay-linux-x64") still encodes the platform tag.
  // Don't read `pkg.name` here — under the npm:alias pattern every variant's
  // package.json has the same `name: "relay"`, so all assets would collide
...
//   2. require.resolve("relay-<platform>-<arch>/package.json")
...
// commands like `relay web` shut down cleanly under Ctrl-C.
// ---------------------------------------------------------------------------
...
const binary = isWin ? "relay.exe" : "relay";
...
const candidates = (() => {
  const base = "relay-" + platform + "-" + arch;
  if (platform === "linux") {
...
const binary = platform === "windows" ? "relay.exe" : "relay";
...
(async () => {
  try {
    ...
    console.log("relay: installed preview " + assetBase + " from " + CDN_BASE);
  } catch (error) {
    console.error("relay preview postinstall failed:", error && error.message ? error.message : error);
    process.exit(1);
  }
})();
`;

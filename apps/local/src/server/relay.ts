...
import { loadPluginsFromJsonc } from "@relay-sh/config";

import executorConfig from "../../relay.config";
...
import * as legacyRelaySchema from "./relay-schema";
import {
...
const localNamespace = "relay_local";
...
const resolveMigrationsFolder = (): string => {
  ...
  const dir = fs.mkdtempSync(join(tmpdir(), "relay-migrations-"));
...
const resolvePluginConfigPath = (scopeDir: string): string => join(scopeDir, "relay.jsonc");

// Plugins reach the host through two doors that compose:
//   - `relay.config.ts`'s static tuple
//   - `relay.jsonc#plugins` loaded at boot
...
const loadLocalPlugins = Effect.gen(function* () {
  ...
  const dedupedDynamic = dynamicPlugins.filter((plugin) => {
    if (plugin.packageName && staticPackageNames.has(plugin.packageName)) {
      console.warn(
        `[relay] plugin "${plugin.packageName}" appears in both ` +
          `relay.config.ts and relay.jsonc#plugins. The static ` +
...
interface LocalRelayBundle {
  readonly relay: Relay<LocalPlugins>;
  readonly plugins: LocalPlugins;
}

class LocalRelayTag extends Context.Service<LocalRelayTag, LocalRelayBundle>()(
  "@relay-sh/local/Relay",
) {}

export type LocalRelay = LocalRelayBundle["relay"];

class LocalRelayCreateError extends Data.TaggedError("LocalRelayCreateError")<{
  readonly operation: "createSqlite" | "importSqlite";
  readonly cause: unknown;
}> {}

class LocalRelayDisposeError extends Data.TaggedError("LocalRelayDisposeError")<{
  readonly operation: "createHandle" | "disposeRelay" | "disposeRuntime";
  readonly cause: unknown;
}> {}

const ignorePromiseFailure = (
  operation: LocalRelayDisposeError["operation"],
  ...
  Effect.runPromise(
    Effect.ignore(
      Effect.tryPromise({
        try: try_,
        catch: (cause) => new LocalRelayDisposeError({ operation, cause }),
      }),
    ),
  );

const handleOrNull = (promise: ReturnType<typeof createRelayHandle>) =>
  Effect.runPromise(
    Effect.tryPromise({
      try: () => promise,
      catch: (cause) => new LocalRelayDisposeError({ operation: "createHandle", cause }),
    }).pipe(
      Effect.catch(() =>
        Effect.succeed<Awaited<ReturnType<typeof createRelayHandle>> | null>(null),
      ),
    ),
  );
...
const isFumaSqliteDatabase = (path: string): boolean => {
  ...
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: native SQLite probe treats unreadable legacy files as non-FumaDB databases
  try {
...
const readSqliteImportMarker = (markerPath: string): SqliteImportMarker | null => {
  ...
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: malformed import markers are treated as incomplete so startup can re-check the database
  try {
...
const replaceSqliteFileSetWithRollback = (input: {
  ...
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: local DB replacement must restore the original file set if the swap fails halfway
  try {
    ...
    // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: preserve the original replacement failure after rollback
    throw cause;
  }
};
...
const prepareLegacySqliteForFumaImport = (input: {
  ...
  if (preScopeBackup) {
    console.warn(
      `[relay] Pre-scope database detected; moved to ${preScopeBackup}. ` +
  ...
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: legacy migration preflight must close SQLite before the FumaDB import re-opens the file
  try {
    if (hasBundledDrizzleMigrationPrefix({ sqlite, migrationsFolder: MIGRATIONS_FOLDER })) {
      sqlite.exec("PRAGMA journal_mode = WAL");
      migrate(drizzle(sqlite, { schema: legacyRelaySchema }), {
        migrationsFolder: MIGRATIONS_FOLDER,
      });
      importLegacySecrets(sqlite, input.scopeId, legacySecrets);
    } else {
      console.warn(
        `[relay] Local SQLite migration history in ${input.storage.dataDir} ` +
...
const importMissingMarkedTables = async (input: {
  ...
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: late plugin-table imports must close the active SQLite handle on failure
  try {
    ...
    await target.close();
    // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: preserve late plugin-table import failure after closing SQLite
    throw cause;
  }
};
...
export const importLegacySqliteIfNeeded = async (options: {
    ...
    if (isFumaSqliteDatabase(storage.sqlitePath)) {
      ...
      if (prepared.preScopeBackup) {
        if (prepared.legacySecrets.length > 0) {
          ...
          // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: pre-scope secret import must close the fresh FumaDB handle on failure
          try {
  ...
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: local SQLite cutover must close and remove the temporary target database on import failure
  try {
    const legacyScopeIds = readLegacySqliteScopeIds({
    ...
    await target.close();
    removeSqliteFileSet(targetPath);
    // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: preserve the original import failure after temp-file cleanup
    throw cause;
  }
};

const createLocalRelayLayer = () => {
  const storage = resolveStorage();

  return Layer.effect(LocalRelayTag)(
    Effect.gen(function* () {
      ...
      const importResult = yield* Effect.tryPromise({
        ...
        catch: (cause) => new LocalRelayCreateError({ operation: "importSqlite", cause }),
      });
      ...
      const sqlite = yield* Effect.acquireRelease(
        Effect.tryPromise({
          ...
          catch: (cause) => new LocalRelayCreateError({ operation: "createSqlite", cause }),
        }),
      ...
      if (importResult.imported) {
        console.warn(
          `[relay] Imported ${importResult.importedRows} row(s) into FumaDB SQLite storage` +
...
export const createRelayHandle = async () => {
  const layer = createLocalRelayLayer();
  const runtime = ManagedRuntime.make(layer);
  const bundle = await runtime.runPromise(LocalRelayTag.asEffect());

  return {
    relay: bundle.relay,
    plugins: bundle.plugins,
    dispose: async () => {
      await Effect.runPromise(Effect.ignore(bundle.relay.close()));
      await ignorePromiseFailure("disposeRuntime", () => runtime.dispose());
    },
  };
};

export type RelayHandle = Awaited<ReturnType<typeof createRelayHandle>>;

let sharedHandlePromise: ReturnType<typeof createRelayHandle> | null = null;

const loadSharedHandle = () => {
  if (!sharedHandlePromise) {
    sharedHandlePromise = createRelayHandle();
  }
  return sharedHandlePromise;
};

export const getRelay = () => loadSharedHandle().then((handle) => handle.relay);
export const getRelayBundle = () => loadSharedHandle();

export const disposeRelay = async (): Promise<void> => {
  ...
  if (handle) {
    await ignorePromiseFailure("disposeRelay", () => handle.dispose());
  }
};

export const reloadRelay = () => {
  disposeRelay();
  return getRelay();
};

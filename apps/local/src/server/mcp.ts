...
import { createRelayMcpServer, type RelayMcpServerConfig } from "@relay-sh/host-mcp";
...
const formatBoundaryError = (error: unknown): unknown => {
  // oxlint-disable-next-line relay/no-instanceof-error, relay/no-unknown-error-message -- boundary: MCP request handler catches unknown SDK/runtime failures for process logging
...
export const createMcpRequestHandler = (config: RelayMcpServerConfig): McpRequestHandler => {
  ...
  return {
    handleRequest: async (request) => {
      ...
      // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: MCP SDK handler must return JSON-RPC errors from thrown Promise APIs
      try {
        created = await Effect.runPromise(createRelayMcpServer(config));
...
export const runMcpStdioServer = async (config: RelayMcpServerConfig): Promise<void> => {
  startIntegrationsRefresh();

  const server = await Effect.runPromise(createRelayMcpServer(config));
  ...
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: stdio server lifetime uses Promise-based SDK/process APIs and always closes resources
  try {

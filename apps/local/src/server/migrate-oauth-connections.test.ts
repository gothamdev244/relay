...
beforeEach(() => {
  workDir = mkdtempSync(join(tmpdir(), "relay-oauth-conn-mig-"));
...
const oauthConnectionMigrationSql = () => {
  ...
  if (start < 0 || end < 0) {
    // oxlint-disable-next-line relay/no-try-catch-or-throw, relay/no-error-constructor -- boundary: test fixture helper must fail fast when the migration section marker changes

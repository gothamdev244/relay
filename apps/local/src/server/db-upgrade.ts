// Pre-scope-refactor relay CLI versions (<= 1.4.x) created a SQLite DB
 ...
 * pre-scope relay — has a `source` table but no `scope_id` column.
...
export const isPreScopeSchema = (dbPath: string): boolean => {
  ...
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: local SQLite schema probe must close the DB handle
  try {
...
export const readLegacySecrets = (dbPath: string): readonly LegacySecret[] => {
  ...
  const db = new Database(dbPath, { readonly: true });
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: local SQLite legacy-row read must close the DB handle
  try {
    const tableExists = db

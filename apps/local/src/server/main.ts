...
import { observabilityMiddleware } from "@relay-sh/api";
import {
  CoreHandlers,
  RelayService,
...
import { getRelayBundle } from "./relay";
...
// into the runtime. The plugin set is the union of `relay.config.ts`
// (static, typed) and `relay.jsonc#plugins` (dynamic, jiti-loaded),
// so `LocalApi` can't be constructed until the relay bundle resolves
...
export const createServerHandlers = async (): Promise<ServerHandlers> => {
  const { relay, plugins } = await getRelayBundle();
  ...
  // satisfied because `composePluginHandlers` provides `relay[id]` to
  // the plugin's own `Layer.succeed(*ExtensionService)(self)` wiring.
  const SpecPluginHandlers = composePluginHandlers(plugins, relay);
...
export class ServerHandlersService extends Context.Service<ServerHandlersService, ServerHandlers>()(
  "@relay-sh/local/ServerHandlersService",
) {}

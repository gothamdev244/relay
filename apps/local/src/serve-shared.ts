...
export const hasBearerToken = (request: Request, token: string): boolean => {
  ...
  return (
    ...
    safeEqual(request.headers.get("x-relay-token") ?? "", token)
  );
};
...
export const hasBasicPassword = (request: Request, password: string): boolean => {
  ...
  let decoded: string;
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: Basic auth decoding accepts untrusted header bytes
  try {

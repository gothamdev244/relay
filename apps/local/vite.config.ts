...
// oxlint-disable-next-line relay/no-json-parse -- boundary: Vite config reads package metadata from package.json
...
const EXECUTOR_GITHUB_URL = (
  ...
  repositoryUrl ??
  "https://github.com/gothamdev244/relay"
)
  .replace(/^git\+/, "")
  .replace(/\.git$/, "");
  ...
  return {
    ...
    configureServer(server) {
      server.watcher.on("change", (path) => {
        if (path.includes("/src/server/") || path.endsWith("/relay.config.ts")) {
          handlers = null;
        }
      });
      server.middlewares.use(async (req, res, next) => {
        ...
        // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: Vite middleware must convert handler failures into HTTP 500 responses
        try {
          if (!handlers) {
...
export default defineConfig({
  ...
  plugins: [
    appPlugin({
      relayConfigPath: resolve(import.meta.dirname, "relay.config.ts"),
      relayJsoncPath: resolve(import.meta.dirname, "relay.jsonc"),
    }),
    executorApiPlugin(),
  ],
});

...
const tracer = trace.getTracer("relay-cloud-worker");
...
const cloudflareHandler: ExportedHandler<Env> = {
  fetch: async (request, env, ctx) => {
    ...
    return tracer.startActiveSpan(`http.server ${request.method}`, async (span) => {
      ...
      // oxlint-disable-next-line relay/no-try-catch-or-throw -- adapter boundary
      try {
        ...
        span.setStatus({ code: SpanStatusCode.ERROR });
        // oxlint-disable-next-line relay/no-try-catch-or-throw -- adapter boundary; preserve original error to Cloudflare runtime
        throw err;
      } finally {
        span.end();

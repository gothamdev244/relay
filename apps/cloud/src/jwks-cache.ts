...
const fetchJwksOnce = async (
  ...
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: fetch adapter must clear abort timer while preserving promise rejection behavior
  try {
    ...
    if (!response.ok) {
      // oxlint-disable-next-line relay/no-try-catch-or-throw, relay/no-error-constructor -- boundary: fetch-backed JWT key resolver must reject with the existing Error cause shape
      throw new Error(`JWKS fetch failed: ${response.status} ${response.statusText}`);
    }

    const body = await response.json();
    // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: fetch JSON validation maps Schema failures to the existing malformed JWKS rejection
    try {
      await decodeJsonWebKeySetPayload(body);
      return body as JSONWebKeySet;
    } catch {
      // oxlint-disable-next-line relay/no-try-catch-or-throw, relay/no-error-constructor -- boundary: fetch JSON validation preserves the existing malformed JWKS rejection
...
export const createCachedRemoteJWKSet = (
  ...
  const get: JWTVerifyGetKey = async (protectedHeader, token) => {
    ...
    // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: jose JWTVerifyGetKey retry path is defined by thrown resolver failures
    try {
      ...
      if (!isJwksNoMatchingKey(error)) {
        // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: jose JWTVerifyGetKey requires preserving upstream resolver rejection
        throw error;
      }

// ---------------------------------------------------------------------------
// Cloud relay — stateless, per-request, new SDK shape
// ---------------------------------------------------------------------------
//
// Each invocation of `createScopedRelay` runs inside a request-scoped
// Effect and yields a fresh relay bound to the current DbService's
...
// ---------------------------------------------------------------------------
// Plugin list lives in `relay.config.ts` — that file is the single source
...
// ---------------------------------------------------------------------------
// Create a fresh relay for a (user, org) pair (stateless, per-request).
//
...
export const createScopedRelay = (
  userId: string,
  ...
  Effect.gen(function* () {
    ...
    // The relay surface returns raw `StorageFailure`; translation to

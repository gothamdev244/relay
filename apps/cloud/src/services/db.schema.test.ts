...
// merge of cloud + relay schemas. If ANY call site builds its own
...
// `combinedSchema = { ...cloudSchema }` and forgot to spread the relay
...
// `./db`) actually contains every relay-schema export AND that drizzle
...
describe("combinedSchema", () => {
  it("spreads every cloud + relay schema export", () => {
    const expected = new Set([...Object.keys(cloudSchema), ...Object.keys(relaySchema)]);
    for (const key of expected) {
      expect(combinedSchema, `combinedSchema missing "${key}"`).toHaveProperty(key);
    }
  });

  // Relay-schema drives the scope-sharded tables (source/tool/etc). If
  // any of these go missing the drizzle adapter's `getTable` lookup throws.
  it("includes scope-sharded relay tables", () => {
    for (const key of Object.keys(relaySchema)) {
      expect(combinedSchema, `combinedSchema missing "${key}"`).toHaveProperty(key);
    }
  });

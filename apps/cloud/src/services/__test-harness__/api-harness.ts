...
import { makeTestWorkOSVaultClient } from "@relay-sh/plugin-workos-vault/testing";
...
// Mirrors apps/cloud/src/services/relay.ts#createScopedRelay — the
...
// that the relay has a valid user-org scope. We give each org a stable
...
// ---------------------------------------------------------------------------
// Relay factory — mirrors apps/cloud/services/executor#createScopedRelay
// but with an in-memory test vault client (see
// `@relay-sh/plugin-workos-vault/testing`).
// ---------------------------------------------------------------------------
...
const createTestScopedRelay = (userId: string, orgId: string, orgName: string) =>
  Effect.gen(function* () {
...
// test-scoped relay against the live postgres test db with a fake
// WorkOS vault, and provides `AuthContext` + the relay services to the
...
const TestExecutionStackMiddleware = HttpRouter.middleware<{
  ...
  Effect.gen(function* () {
    ...
    const provideRelayExtensions = providePluginExtensions(testPlugins);
    return (httpEffect) =>
      Effect.gen(function* () {
        const request = yield* HttpServerRequest.HttpServerRequest;
        const orgId = request.headers[TEST_ORG_HEADER];
        if (!orgId || typeof orgId !== "string") {
          // oxlint-disable-next-line relay/no-effect-escape-hatch, relay/no-error-constructor -- boundary: test HTTP harness has no request context without x-test-org-id
        ...
        const relay = yield* createTestScopedRelay(userId, orgId, orgName);

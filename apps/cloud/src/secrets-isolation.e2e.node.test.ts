...
// builds the same shape `apps/cloud/src/services/relay.ts#createScopedRelay`
...
//   5. secrets.set rejects a scope id outside the caller's relay stack.
//
// NOTE: "per-user override shadows org default" cross-scope co-existence
// is NOT covered here. `relay.secrets.set` currently deletes secret
// metadata rows across the full scope stack before re-inserting at the
// target scope (see relay.ts `secretsSet`), so an overrider writing
...
import { ScopeId, SecretId } from "@relay-sh/sdk";
...
describe("cloud secret isolation (HTTP, user-org scope stack)", () => {
  ...
  it.effect("secrets.set rejects a scope outside the relay's stack", () =>
    Effect.gen(function* () {
      const orgId = nextOrgId();
      const userId = nextUserId();

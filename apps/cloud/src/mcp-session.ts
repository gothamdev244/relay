...
import { createRelayMcpServer } from "@relay-sh/host-mcp";
import { buildExecuteDescription } from "@relay-sh/execution";
...
const INTERNAL_ACCOUNT_ID_HEADER = "x-relay-mcp-account-id";
const INTERNAL_ORGANIZATION_ID_HEADER = "x-relay-mcp-organization-id";
...
const makeDbHandle = (options: {
  ...
  return {
    ...
    // oxlint-disable-next-line relay/no-promise-catch -- boundary: postgres.js close is best-effort during DO/runtime cleanup
...
export class McpSessionDO extends DurableObject {
  ...
  private clearSessionState(): Effect.Effect<void> {
    return Effect.promise(async () => {
      ...
      await Promise.all([
        // oxlint-disable-next-line relay/no-promise-catch -- boundary: Durable Object storage cleanup is best-effort after session invalidation
        ...
        // oxlint-disable-next-line relay/no-promise-catch -- boundary: Durable Object alarm cleanup is best-effort after session invalidation
  ...
  private createConnectedRuntime(
    ...
    return Effect.gen(function* () {
      const { relay, engine } = yield* makeExecutionStack(
        sessionMeta.userId,
        sessionMeta.organizationId,
        sessionMeta.organizationName,
      );
      // Build the description here so the postgres query it runs
      // (`relay.sources.list`) lands as a child of
      ...
      const mcpServer = yield* createRelayMcpServer({
        engine,
        description,
        parentSpan: () => self.currentRequestSpan ?? undefined,
        debug: env.EXECUTOR_MCP_DEBUG === "true",
      }).pipe(Effect.withSpan("McpSessionDO.createRelayMcpServer"));
  ...
  private closeRuntime(): Effect.Effect<void> {
    ...
    return Effect.gen(function* () {
      ...
      if (self.mcpServer) {
        ...
        // oxlint-disable-next-line relay/no-promise-catch -- boundary: MCP SDK close failure is ignored during best-effort runtime teardown
      ...
      // oxlint-disable-next-line relay/no-effect-escape-hatch -- boundary: DO cleanup has no typed failure surface
      Effect.orDie,
    );
  }
  ...
  private restoreRuntimeFromStorage(request: Request): Effect.Effect<"restored" | "missing_meta"> {
    ...
    return Effect.gen(function* () {
      ...
      // oxlint-disable-next-line relay/no-effect-escape-hatch -- boundary: cold DO restore is re-entered from Promise-only Durable Object method
      Effect.orDie,
    );
  }
  ...
  private ensureJsonResponseTransportForPost(request: Request): Effect.Effect<void> {
    ...
    return Effect.gen(function* () {
      ...
      // oxlint-disable-next-line relay/no-effect-escape-hatch -- boundary: transport rebuild is internal DO runtime state
      Effect.orDie,
    );
  }
  ...
  async init(token: McpSessionInit, incoming?: IncomingTraceHeaders): Promise<void> {
    ...
    return Effect.runPromise(
      Effect.gen(function* () {
        ...
        // oxlint-disable-next-line relay/no-effect-escape-hatch -- boundary: Durable Object init method can only reject its Promise
        Effect.orDie,
      ),
    );
  }
  ...
  private doInit(token: McpSessionInit) {
    ...
    // createRuntime, createScopedRelay, createRelayMcpServer,
    ...
    return Effect.gen(function* () {
      ...
      // oxlint-disable-next-line relay/no-effect-escape-hatch -- boundary: doInit is called only from Promise-only Durable Object init
      Effect.orDie,
    );
  }

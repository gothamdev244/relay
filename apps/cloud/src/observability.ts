...
//
// `withObservability` (in @relay-sh/api) wraps every handler effect; when
...
import { ErrorCapture } from "@relay-sh/api";
...
export const sentryPayloadForCause = (
  ...
  if (Cause.isCause(input)) {
    ...
    // oxlint-disable-next-line relay/no-error-constructor -- boundary: Sentry captureException needs an Error-like primary payload for pretty Effect causes

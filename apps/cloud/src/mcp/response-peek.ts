...
const jsonRpcResponseAttrs = (payload: JsonRpcResponseBody | null): Record<string, unknown> => {
  ...
  if (err && typeof err === "object") {
    ...
    // oxlint-disable-next-line relay/no-unknown-error-message -- boundary: schema-decoded JSON-RPC error message is protocol telemetry
    const { message } = err;
...
const readResponseText = async (response: Response, timeoutMs: number | null): Promise<string> => {
  ...
  const timeoutPromise = new Promise<never>((_, reject) => {
    timeout = setTimeout(() => {
      // oxlint-disable-next-line relay/no-promise-catch -- boundary: best-effort stream cancellation inside timeout callback
      void reader.cancel().catch(() => undefined);
      // oxlint-disable-next-line relay/no-promise-reject -- boundary: Promise.race timeout adapter for Web ReadableStream
      reject(new ResponseBodyTimeoutError({ timeoutMs }));
    }, timeoutMs);
  });
  const readPromise = (async () => {
    // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: Web stream reader cleanup must clear host timeout after success or failure
    try {
      let text = "";
      for (;;) {

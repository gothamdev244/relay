...
//   - `createScopedRelay` against a real FumaDB/Drizzle handle (the 2026-04-16
//     prod outage was a schema spread bug here; see services/db.schema.test.ts)
//   - `createExecutionEngine` with an in-process code relay
//   - `createRelayMcpServer` for the MCP request surface
...
const buildScopedRelay = (scopeId: string, scopeName: string, options: BuildOptions = {}) =>
  Effect.gen(function* () {
...
const openSession = (
  ...
  Effect.acquireRelease(
    Effect.gen(function* () {
      const relay = yield* buildScopedRelay(orgId, `Org ${orgId}`, options);
      const engine = createExecutionEngine({ relay, codeRelay: makeQuickJsRelay() });
      const mcpServer = yield* createRelayMcpServer({ engine });
...
describe("cloud MCP session end-to-end", () => {
  ...
  // thrown at `relay.sources.list()` when the MCP session's drizzle
  // instance lost the relay-schema tables.
  it.effect("exercises the drizzle adapter directly via relay.sources.list", () =>
    Effect.gen(function* () {
      const relay = yield* buildScopedRelay(nextOrgId(), "drizzle-probe");

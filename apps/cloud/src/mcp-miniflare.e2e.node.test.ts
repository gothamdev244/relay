...
// `tools.relay.openapi.addSource`, and the cloud engine invokes a POST operation.
// `requiresApproval: true` fires the relay's approval elicitation, which
...
import { serveOpenApiHttpApiTestServer } from "@relay-sh/plugin-openapi/testing";
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(client.getServerVersion()?.name).toBe("relay");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // `enforceApproval` in the relay; that goes through the MCP
        ...
        const code = [
          `await tools.relay.openapi.addSource({ scope: ${JSON.stringify(orgId)}, spec: ${JSON.stringify(specJson)}, namespace: "approveapi" });`,

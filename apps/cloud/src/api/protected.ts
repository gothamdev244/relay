...
// relay[id])` chain. The plugin spec carries the Service tag so
// this file doesn't import each plugin's `*/api` directly.
const provideRelayExtensions = providePluginExtensions(cloudPlugins);
...
//   4. builds the per-request relay + engine,
...
const ExecutionStackMiddleware = HttpRouter.middleware<{
  ...
  // `providePluginExtensions(cloudPlugins)(relay)` below.
  provides:
    | AuthContext
    | RelayService
    | ExecutionEngineService

...
import { createRelay, type Tool } from "@relay-sh/sdk";
...
export const POST: APIRoute = async ({ request }) => {
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: Astro route converts request/parsing failures to a stable HTTP response
  try {
    ...
    // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: URL constructor is the platform validator for request input
    try {
      new URL(url);
    } catch {
    ...
    const program = Effect.gen(function* () {
      ...
      // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: ensure relay cleanup runs after best-effort marketing detection
      try {
        // Detect what kind of source lives at this URL
        const detected = yield* relay.sources.detect(url).pipe(Effect.timeout("10 seconds"));

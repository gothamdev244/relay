 ...
 *   2. Spawn the compiled `relay-sidecar` binary with EXECUTOR_PORT=0
...
const BINARY = resolve(
  ROOT,
  "resources/sidecar",
  process.platform === "win32" ? "relay-sidecar.exe" : "relay-sidecar",
);

const AUTH_PASSWORD = "smoke-test-password";
const AUTH_HEADER = `Basic ${btoa(`relay:${AUTH_PASSWORD}`)}`;
...
const waitForReadyPort = (proc: Subprocess<"ignore", "pipe", "pipe">): Promise<number> =>
  // oxlint-disable-next-line relay/no-promise-reject -- boundary: standalone build-time smoke harness, no Effect runtime
  new Promise((resolveReady, rejectReady) => {
    const deadline = setTimeout(() => {
      // oxlint-disable-next-line relay/no-promise-reject, relay/no-error-constructor -- boundary: standalone smoke harness reporting a build-time timeout
    ...
    void (async () => {
      while (true) {
        ...
        if (done) {
          ...
          // oxlint-disable-next-line relay/no-promise-reject, relay/no-error-constructor -- boundary: standalone smoke harness, stdout-closed surfaced as rejection
...
const main = async () => {
  ...
  const scopeDir = await mkdtemp(join(tmpdir(), "relay-smoke-"));
  ...
  const cleanup = async () => {
    ...
    // oxlint-disable-next-line relay/no-promise-catch -- boundary: best-effort tempdir cleanup in a standalone smoke harness
    await rm(scopeDir, { recursive: true, force: true }).catch(() => {});
  };

  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: standalone smoke harness needs a finally to tear down the spawned binary + http server
  try {
    ...
    // inside QuickJS.
    const code = `
await tools.relay.openapi.addSource({

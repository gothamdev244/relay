 ...
 * production).
 *
 * Reads connection parameters from env, boots the relay server, then
...
import { startServer } from "@relay-sh/local";
...
if (!authPassword) {
  // oxlint-disable-next-line relay/no-try-catch-or-throw, relay/no-error-constructor -- boundary: misconfiguration at sidecar boot is fatal
...
const stop = async (code: number) => {
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: shutdown path must terminate even when stop() throws
  try {
    await server.stop();
  } finally {
    process.exit(code);

...
const api = {
  ...
  getSettings(): Promise<DesktopServerSettings> {
    return ipcRenderer.invoke("relay:settings:get");
  },
  ...
  restartServer(): Promise<{ readonly port: number; readonly baseUrl: string }> {
    return ipcRenderer.invoke("relay:server:restart");
  },
  /**
  ...
  openExternal(url: string): Promise<void> {
    return ipcRenderer.invoke("relay:shell:open-external", url);
  },
} as const;

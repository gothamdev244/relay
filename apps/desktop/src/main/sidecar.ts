 ...
 * In prod: spawns the Bun-compiled `relay-sidecar` binary shipped under
 *          `process.resourcesPath/sidecar/`.
 *
 * Either way, the child receives EXECUTOR_PORT/EXECUTOR_HOST/EXECUTOR_AUTH_PASSWORD
 * via env, calls `startServer()` from `@relay-sh/local`, and announces a
...
const resolveSidecarCommand = (): { command: string; args: string[]; cwd: string } => {
  if (app.isPackaged) {
    const binaryName = process.platform === "win32" ? "relay-sidecar.exe" : "relay-sidecar";
...
export async function startSidecar(options: StartOptions = {}): Promise<SidecarConnection> {
  ...
  if (!existsSync(clientDir)) {
    // oxlint-disable-next-line relay/no-try-catch-or-throw, relay/no-error-constructor -- boundary: misconfiguration is fatal
    throw new Error(
      `Relay client bundle not found at ${clientDir}. Run \`bun run --filter @relay-sh/local build\` before launching desktop.`,
    );
  }

  // data.db and the optional relay.jsonc plugin manifest live under
  // ~/.relay — the same path the CLI's `relay web` uses. Desktop and CLI
  ...
  // `relay mcp --scope ~/.relay` stay copy-paste-friendly. Electron's
  ...
  const scopeDir = join(homedir(), ".relay");
  ...
  return new Promise<SidecarConnection>((resolveStart, rejectStart) => {
    ...
    const reject = (err: Error) => {
      ...
      rejected = true;
      // oxlint-disable-next-line relay/no-promise-reject -- boundary: sidecar startup surfaces as a rejected promise
      rejectStart(err);
    };

    const onStdout = (chunk: Buffer) => {
      const text = chunk.toString("utf8");
      process.stdout.write(`[relay-sidecar] ${text}`);
    ...
    const onStderr = (chunk: Buffer) => {
      const text = chunk.toString("utf8");
      stderrBuffer += text;
      process.stderr.write(`[relay-sidecar] ${text}`);
    };
    ...
    const onExit = (code: number | null, signal: NodeJS.Signals | null) => {
      ...
      // oxlint-disable-next-line relay/no-error-constructor -- boundary: sidecar boot failure surfaces here as a rejected start promise

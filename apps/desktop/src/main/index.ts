...
// at a predictable spot. User-mutable relay state (data.db and the optional
// relay.jsonc plugin manifest) is pinned separately to ~/.relay in
...
/**
 * Resolve the on-disk path to the Relay app icon. Packaged builds get
...
const showPortInUseDialog = async (port: number) => {
  await dialog.showMessageBox({
    type: "error",
    title: "Relay port in use",
    message: `Port ${port} is already taken.`,
    detail:
      "Another process is listening on that port. Quit it (or change the desktop server's port in Settings) and relaunch Relay.",
    buttons: ["OK"],
  });
};

const startWithCurrentSettings = async (): Promise<SidecarConnection | null> => {
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: bind failures surface as a user-facing dialog
  try {
    return await startSidecar();
  } catch (error) {
    // oxlint-disable-next-line relay/no-instanceof-tagged-error -- boundary: SidecarPortInUseError is a plain Node Error subclass, not an Effect tagged error
    if (error instanceof SidecarPortInUseError) {
      await showPortInUseDialog(error.port);
      return null;
    }
    log.error("Failed to start relay sidecar", error);
    return null;
  }
};
...
const restartSidecarAndReload = async (): Promise<{ port: number; baseUrl: string }> => {
  ...
  if (!next) {
    // oxlint-disable-next-line relay/no-try-catch-or-throw, relay/no-error-constructor -- boundary: surfaces to renderer as a rejected IPC call
...
const registerIpcHandlers = () => {
  ipcMain.handle("relay:settings:get", (): DesktopServerSettings => getServerSettings());
  ipcMain.handle(
  ...
  ipcMain.handle(
    "relay:settings:regenerate-password",
    (): DesktopServerSettings => regeneratePassword(),
  );
  ipcMain.handle("relay:server:restart", () => restartSidecarAndReload());
  ipcMain.handle("relay:shell:open-external", async (_evt, rawUrl: unknown) => {
    if (typeof rawUrl !== "string") return;
    // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: untrusted renderer string, URL ctor throws on malformed input
    try {
...
const promptInstallUpdate = async (version: string) => {
  ...
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: dialog wrapper guarantees the open flag clears
  try {
    const response = await dialog.showMessageBox({
      type: "info",
      title: "Update ready",
      message: `Relay ${version} is ready to install.`,
...
const runUpdateCheck = async ({ alertOnFail }: UpdateCheckOptions) => {
  ...
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: surface network/update failures only when the user asked
  try {

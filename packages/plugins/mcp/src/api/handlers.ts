...
import { addGroup, capture } from "@relay-sh/api";
import { ScopeId } from "@relay-sh/sdk/core";
import type {
  McpPluginExtension,
...
// ---------------------------------------------------------------------------
// Service tag — holds the raw extension shape the relay produces.
// Handlers wrap their generator bodies with `capture(...)` from
// `@relay-sh/api`, which translates `StorageError` to `InternalError`
...
const RelayApiWithMcp = addGroup(McpGroup);
...
export const McpHandlers = HttpApiBuilder.group(RelayApiWithMcp, "mcp", (handlers) =>
  handlers

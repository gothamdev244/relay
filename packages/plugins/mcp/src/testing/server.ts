...
import { OAuthTestServer } from "@relay-sh/sdk/testing";
import z from "zod";
...
export class McpTestServerLayer extends Context.Service<McpTestServerLayer, McpTestServer>()(
  "@relay-sh/plugin-mcp/testing/McpTestServer",
) {
...
export const makeGreetingMcpServer = (
  ...
  const server = new McpServer(
    {
      name: options.name ?? "relay-test-mcp",
  ...
  server.registerTool(
    ...
    {
      description: options.toolDescription ?? "Echoes from the relay MCP test server",
      inputSchema: {},
    },
    async () => ({
...
export const makeEchoMcpServer = (
  ...
  const server = new McpServer(
    {
      name: options.name ?? "relay-echo-mcp",
      version: options.version ?? "1.0.0",
    },
    { capabilities: {} },
  );

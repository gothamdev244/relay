...
// `relay.connections.create` has nowhere to persist its owned
...
describe("mcpPlugin", () => {
  it.effect("creates relay with mcp plugin", () =>
    Effect.gen(function* () {
      ...
      expect(relay.mcp).toBeDefined();
      expect(relay.mcp.addSource).toBeTypeOf("function");
      expect(relay.mcp.removeSource).toBeTypeOf("function");
      expect(relay.mcp.refreshSource).toBeTypeOf("function");
      expect(relay.mcp.probeEndpoint).toBeTypeOf("function");
      expect(relay.oauth.start).toBeTypeOf("function");
      expect(relay.oauth.complete).toBeTypeOf("function");
    }),
  );

  it.effect("sources list is initially empty", () =>
    Effect.gen(function* () {
      const relay = yield* createRelay(makeTestConfig({ plugins: [mcpPlugin()] as const }));
  ...
  // `seedShadowed` wraps `relay.mcp.addSource` at a given scope with
  ...
  it.effect("shadowed addSource does not wipe the outer-scope source", () =>
    Effect.gen(function* () {
      ...
      yield* seedShadowed(relay.mcp.addSource, {
        scope: ORG_SCOPE,
        name: "Org Source",
      ...
      const userView = yield* relay.mcp.getSource("shared", USER_SCOPE);
      const orgView = yield* relay.mcp.getSource("shared", ORG_SCOPE);
  ...
  it.effect("removeSource on user shadow leaves the org row intact", () =>
    Effect.gen(function* () {
      ...
      yield* relay.mcp.removeSource("shared", USER_SCOPE);
  ...
  it.effect("updateSource on user shadow does not mutate the org row", () =>
    Effect.gen(function* () {
      ...
      yield* relay.mcp.updateSource("shared", USER_SCOPE, {
        name: "User Renamed",
  ...
  it.effect("updateSource removes bindings for credential slots no longer present", () =>
    Effect.gen(function* () {
      ...
      yield* relay.mcp.updateSource("stale_binding", "test-scope", {
        auth: { kind: "none" },
      });

      const bindings = yield* relay.mcp.listSourceBindings("stale_binding", "test-scope");
  ...
  it.effect("updateSource writes auth changes back to the config file", () =>
    Effect.gen(function* () {
      ...
      yield* relay.mcp.updateSource("sentry", "test-scope", {
  ...
  it.effect("addSource accepts oauth2 auth with no backing connection", () =>
    Effect.gen(function* () {
      ...
      const stored = yield* relay.mcp.getSource("deferred_oauth", "test-scope");
  ...
  it.effect("source renders in needs-auth state when no connection exists", () =>
    Effect.gen(function* () {
      ...
      const connections = yield* relay.connections.list();
      const connectionMatch = connections.find((c) => c.id === "mcp-oauth2-needs_auth");
      expect(connectionMatch).toBeUndefined();

      const stored = yield* relay.mcp.getSource("needs_auth", "test-scope");
      expect(stored?.config.transport).toBe("remote");
      if (stored?.config.transport !== "remote") return;
      expect(stored.config.auth.kind).toBe("oauth2");
    }),
  ...
  it.effect("signing in as a user transitions the source to connected", () =>
    Effect.gen(function* () {
      ...
      const pre = yield* relay.connections.list();
      ...
      yield* relay.mcp.setSourceBinding({
        sourceId: "team_mcp",
      ...
      const post = yield* relay.connections.list();
      const match = post.find((c) => c.id === "mcp-oauth2-team_mcp");
      expect(match).toBeDefined();
      expect(match?.scopeId).toBe(USER_SCOPE_ID);

      const accessToken = yield* relay.connections.accessToken(connectionId);
      ...
      const stored = yield* relay.mcp.getSource("team_mcp", ORG_SCOPE_ID);
      expect(stored?.config.transport).toBe("remote");
      if (stored?.config.transport !== "remote") return;
      expect(stored.config.auth.kind).toBe("oauth2");
      if (stored.config.auth.kind !== "oauth2") return;
  ...
  it.effect("usagesForSecret aggregates header-auth + headers child rows", () =>
    Effect.gen(function* () {
      ...
      const usages = yield* relay.secrets.usages(SecretId.make("shared-key"));
      ...
      const otherUsages = yield* relay.secrets.usages(SecretId.make("other-secret"));
  ...
  it.effect("usagesForConnection finds oauth2-bound mcp sources", () =>
    Effect.gen(function* () {
      ...
      const usages = yield* relay.connections.usages(ConnectionId.make("conn-xyz"));
...
describe("mcpPlugin detect URL-token fallback", () => {
  ...
  it.effect("returns low-confidence candidate when path has /mcp segment", () =>
    Effect.gen(function* () {
      const relay = yield* createRelay(makeTestConfig({ plugins: [mcpPlugin()] as const }));
      const results = yield* relay.sources.detect("http://127.0.0.1:1/api/mcp");
  ...
  it.effect("matches mcp on hostname label", () =>
    Effect.gen(function* () {
      const relay = yield* createRelay(makeTestConfig({ plugins: [mcpPlugin()] as const }));
      const results = yield* relay.sources.detect("http://mcp.127.0.0.1.nip.io:1/");
      const mcp = results.find((r) => r.kind === "mcp");
      expect(mcp?.confidence).toBe("low");
    }),
  );
  ...
  it.effect("does not match mcp as a substring", () =>
    Effect.gen(function* () {
      ...
      const results = yield* relay.sources.detect("http://127.0.0.1:1/mcpstore");

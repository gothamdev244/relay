...
/**
 * Relay scope id that owns a newly-added MCP source row. Must be one
 * of the relay's configured scopes. Admins adding a shared server at
...
// `/scopes/:scopeId/oauth/*` surface in `@relay-sh/api` — no
    ...
    if (sourceSourceRank === Infinity) {
      return yield* new StorageError({
        message:
          ...
          `which is not in the relay's scope stack ${scopeList}.`,
        cause: undefined,
      });
    }
    if (targetRank === Infinity) {
      return yield* new StorageError({
        message:
          ...
          `in the relay's scope stack ${scopeList}.`,
        cause: undefined,
      });
    }
...
const makeOAuthProvider = (accessToken: string): OAuthClientProvider => ({
  ...
  redirectToAuthorization: async () => {
    // oxlint-disable-next-line relay/no-try-catch-or-throw, relay/no-error-constructor -- boundary: MCP SDK OAuthClientProvider callback can only signal reauthorization by throwing
    throw new Error("MCP OAuth re-authorization required");
  },
  saveCodeVerifier: () => undefined,
  codeVerifier: () => {
    // oxlint-disable-next-line relay/no-try-catch-or-throw, relay/no-error-constructor -- boundary: MCP SDK OAuthClientProvider callback requires a thrown verifier failure
...
export interface McpPluginOptions {
  ...
  /** If provided, source add/remove is mirrored to relay.jsonc
...
export const mcpPlugin = definePlugin((options?: McpPluginOptions) => {
  ...
  // connection cache across a single createRelay lifecycle.
  ...
  return {
    id: "mcp" as const,
    packageName: "@relay-sh/plugin-mcp",
    // Surfaced to the client bundle via the Vite plugin (see
    // `@relay-sh/vite-plugin`). The MCP `./client` factory reads
    ...
    invokeTool: ({ ctx, toolRow, args, elicit }) =>
      Effect.gen(function* () {
        ...
        // (innermost-wins from the relay's stack). The matching
  ...
  // the api-aware factory in `@relay-sh/plugin-mcp/api`. Hosts that
  // want the HTTP surface import the plugin from there; SDK-only
  // consumers stay on this entry and avoid the server-only deps.
});

// ---------------------------------------------------------------------------
// McpPluginExtension — shape of `relay.mcp` for consumers that want
 ...
 * the HTTP edge (`@relay-sh/api`'s `withCapture`) translates

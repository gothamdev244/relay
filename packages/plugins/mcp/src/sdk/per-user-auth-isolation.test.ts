...
const makeLayeredMcpRelays = () =>
  Effect.acquireRelease(
    Effect.gen(function* () {
      ...
      const execUserA = yield* createRelay({
        ...config,
        scopes: [userAScope, orgScope],
      });
      const execUserB = yield* createRelay({
        ...config,
...
describe("per-user MCP auth isolation", () => {
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const { execUserA, execUserB } = yield* makeLayeredMcpRelays();
  ...
  it.effect("org header binding resolves the org secret when a user has the same secret id", () =>
    Effect.gen(function* () {
      const server = yield* serveAuthRecordingMcpServer;
      const { execUserA } = yield* makeLayeredMcpRelays();

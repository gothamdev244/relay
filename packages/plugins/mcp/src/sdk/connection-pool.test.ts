...
// `plugin.ts#makeRuntime`), so within a single relay / session DO the
...
// ---------------------------------------------------------------------------
// Helper — one relay, one mcp source pointed at the test server
// ---------------------------------------------------------------------------

const makeTestRelay = (serverUrl: string, namespace = "pool-test") =>
  createRelay(
    makeTestConfig({
...
describe("MCP connection pooling (regression)", () => {
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* relay.tools.invoke(echo.id, { value: "1" }, { onElicitation: "accept-all" });
  ...
  it.effect("different tools on the same source share the cached connection", () =>
    Effect.gen(function* () {
      ...
      yield* relay.tools.invoke(echo.id, { value: "a" }, { onElicitation: "accept-all" });
      ...
      yield* relay.tools.invoke(echo2.id, { value: "b" }, { onElicitation: "accept-all" });
      expect(server.sessionCount()).toBe(baseline);

      // Back to the first tool — still pooled.
      yield* relay.tools.invoke(echo.id, { value: "c" }, { onElicitation: "accept-all" });
      expect(server.sessionCount()).toBe(baseline);
    }),
  );

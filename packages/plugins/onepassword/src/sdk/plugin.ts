...
// ---------------------------------------------------------------------------
// Plugin extension — public API on relay.onepassword
// ---------------------------------------------------------------------------
...
const makeProvider = (
  ...
  // 1Password vaults are named in the stored config; the relay-scope
...
export const onepasswordPlugin = definePlugin((options?: OnePasswordPluginOptions) => {
  ...
  return {
    ...
    packageName: "@relay-sh/plugin-onepassword",
  ...
  // the api-aware factory in `@relay-sh/plugin-onepassword/api`. Hosts

/* oxlint-disable react/forbid-elements -- plugin component uses raw HTML controls per SDK convention; see @relay-sh/plugin-example for the same pattern */
/**
 * @relay-sh/plugin-desktop-settings/client
 *
 ...
 * main process via `window.relay.*` (exposed by `apps/desktop/src/preload`).
 *
 * The plugin is bundled into apps/local's renderer too (because relay
 * web + desktop share the same client bundle pipeline), but the page
 * only registers a nav entry when `window.relay` is present at
...
// imported from @relay-sh/plugin-desktop-settings (or a shared package)
// so this client bundle has no runtime dependency on the Electron
// surface — when `window.relay` is undefined (web), the page silently
...
const readBridge = (): RelayBridge | null => {
  if (typeof window === "undefined") return null;
  const candidate = (window as Window & { readonly relay?: RelayBridge }).relay;
...
function SettingsPage() {
  ...
  const apply = useCallback(
    async (patch: Partial<DesktopServerSettings>) => {
      ...
      // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: renderer ↔ Electron IPC, errors surface in the form
      try {
      ...
      // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: same as above
      try {
  ...
  const regenerate = useCallback(async () => {
    ...
    setStatus("saving");
    let next: DesktopServerSettings;
    // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: renderer ↔ Electron IPC
    try {
    ...
    return (
      <div style={{ maxWidth: 560, margin: "3rem auto", padding: "1.5rem" }}>
        ...
        <p style={{ color: "var(--muted-foreground, #888)" }}>
          This panel configures the Relay Desktop app's local server. Open this page from the

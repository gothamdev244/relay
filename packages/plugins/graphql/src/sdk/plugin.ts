...
const namespaceFromEndpoint = (endpoint: string): string => {
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: URL construction throws; this helper intentionally falls back to the stable default namespace
  try {
...
export const graphqlPlugin = definePlugin((options?: GraphqlPluginOptions) => {
  return {
    id: "graphql" as const,
    packageName: "@relay-sh/plugin-graphql",
  ...
  // the api-aware factory in `@relay-sh/plugin-graphql/api`. Hosts that

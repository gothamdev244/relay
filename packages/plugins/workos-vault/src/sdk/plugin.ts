...
// client; the scope id is threaded in per-call by the relay's
// secrets facade.
...
export const workosVaultPlugin = definePlugin((options?: WorkOSVaultPluginOptions) => ({
  ...
  packageName: "@relay-sh/plugin-workos-vault",
  ...
  secretProviders: (ctx) => {
    ...
    // which is what we want — the relay fails to start rather

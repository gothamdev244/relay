...
/* oxlint-disable relay/no-try-catch-or-throw -- boundary: fake WorkOS Promise client throws SDK-like errors so wrapper behavior can be tested */
...
import { makeTestConfig, makeTestRelay } from "@relay-sh/sdk/testing";
...
const makeRelay = (client: WorkOSVaultClient) =>
  makeTestRelay({ plugins: [workosVaultPlugin({ client })] as const });

describe("WorkOS Vault secret provider", () => {
  it.effect("stores and resolves secrets through WorkOS Vault", () =>
    Effect.gen(function* () {
      const relay = yield* makeRelay(makeFakeClient());
      ...
      expect(yield* relay.secrets.get("github-token")).toBe("ghp_secret");
      expect(relay.workosVault.providerKey).toBe("workos-vault");

      const listed = yield* relay.secrets.list();
  ...
  it.effect("updates metadata and secret values in place", () =>
    Effect.gen(function* () {
      ...
      expect(yield* relay.secrets.get("api-key")).toBe("v2");
  ...
  it.effect("removes secrets from Vault and metadata store", () =>
    Effect.gen(function* () {
      ...
      expect(yield* relay.secrets.get("remove-me")).toBe("gone soon");
      ...
      expect(yield* relay.secrets.get("remove-me")).toBeNull();
      expect(yield* relay.secrets.list()).toHaveLength(0);
    }),
  );
  ...
  it.effect("treats invalid Vault object names as missing during removal", () =>
    Effect.gen(function* () {
      ...
      const relay = yield* makeRelay(client);
  ...
  it.effect("retries secret value writes on 409 version conflicts", () =>
    Effect.gen(function* () {
      const relay = yield* makeRelay(makeFakeClient({ conflictOnNextSecretUpdate: true }));
      ...
      expect(yield* relay.secrets.get("conflict")).toBe("retry-me");
...
const makeLayeredRelays = (client: WorkOSVaultClient) =>
  Effect.gen(function* () {
    ...
    const execOuter = yield* createRelay({ ...config, scopes: [outerScope] });
    const execInner = yield* createRelay({ ...config, scopes: [innerScope, outerScope] });
...
describe("WorkOS Vault secret provider — multi-scope isolation", () => {
  it.effect("encodes personal scope ids before using them in Vault object names", () =>
    Effect.gen(function* () {
      ...
      const { execInner, innerId } = yield* makeLayeredRelays(client);
  ...
  it.effect("secrets.remove at the inner scope does not wipe outer-scope metadata", () =>
    Effect.gen(function* () {
      ...
      const { execOuter, execInner, outerId, innerId } = yield* makeLayeredRelays(client);
  ...
  it.effect("list only includes metadata from the relay scope stack", () =>
    Effect.gen(function* () {
      const client = makeFakeClient();
      const plugins = [workosVaultPlugin({ client })] as const;
      ...
      const execA = yield* createRelay({ ...config, scopes: [orgA] });
      const execB = yield* createRelay({ ...config, scopes: [orgB] });
...
const makeRelayForScope = (client: WorkOSVaultClient, scopeId: string) =>
  makeTestRelay({
...
describe("WorkOS Vault secret provider — KEK context", () => {
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const relay = yield* makeRelayForScope(recording, "user-org:u1:org42");
  ...
  it.effect("falls back to `{app, organization_id: scopeId}` for bare scope ids", () =>
    Effect.gen(function* () {
      ...
      const relay = yield* makeRelayForScope(recording, "org42");

...
// oxlint-disable-next-line relay/no-error-constructor -- boundary: test injects a defect to verify opaque handler error responses
...
const UnusedRelay = Layer.succeed(RelayService)({} as RelayService["Service"]);
const UnusedExecutionEngine = Layer.succeed(ExecutionEngineService)(
  {} as ExecutionEngineService["Service"],
);
const HandlerContext = Context.make(RelayService, {} as RelayService["Service"]).pipe(
...
describe("GoogleDiscoveryHandlers", () => {
  it.effect("encodes stored source details returned from the SDK store", () =>
    Effect.gen(function* () {
      ...
      const context = Context.make(RelayService, {} as RelayService["Service"]).pipe(
        Context.add(ExecutionEngineService, {} as ExecutionEngineService["Service"]),

...
// `Layer.succeed(GoogleDiscoveryExtensionService, withCapture(relay.googleDiscovery))`.
...
const RelayApiWithGoogleDiscovery = addGroup(GoogleDiscoveryGroup);
...
// group in `@relay-sh/api` now — the plugin has no OAuth-specific handlers.
// ---------------------------------------------------------------------------

export const GoogleDiscoveryHandlers = HttpApiBuilder.group(
  RelayApiWithGoogleDiscovery,
  "googleDiscovery",
  (handlers) =>
    handlers

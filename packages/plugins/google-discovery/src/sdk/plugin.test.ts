...
const startServer = (): Promise<ServerHandle> =>
  new Promise((resolvePromise, rejectPromise) => {
    ...
    server.listen(0, "127.0.0.1", (error?: Error) => {
      if (error) {
        // oxlint-disable-next-line relay/no-promise-reject -- boundary: node listen callback reports startup failure through Promise adapter
        rejectPromise(error);
        return;
      }
      const address = server.address();
      if (!address || typeof address === "string") {
        // oxlint-disable-next-line relay/no-promise-reject, relay/no-error-constructor -- boundary: node listen callback reports startup failure through Promise adapter
      ...
      resolvePromise({
        ...
        close: () =>
          new Promise((resolveClose, rejectClose) => {
            // oxlint-disable-next-line relay/no-promise-reject -- boundary: node close callback reports shutdown failure through Promise adapter
...
// `relay.secrets.set` / `ctx.secrets.set`. Without this there's no
// writable provider registered against the test relay.
// ---------------------------------------------------------------------------

import { definePlugin, type SecretProvider } from "@relay-sh/sdk";
...
describe("Google Discovery plugin", () => {
  it.effect("normalizes legacy googleapis discovery urls", () =>
    Effect.gen(function* () {
      const relay = yield* Effect.acquireRelease(
        ...
        (relay) => relay.close().pipe(Effect.ignore),
      );
      ...
      const result = yield* relay.googleDiscovery.probeDiscovery(
  ...
  it.effect("starts oauth using caller-supplied discovery scopes", () =>
    Effect.gen(function* () {
      ...
      const result = yield* relay.oauth.start({
  ...
  it.effect("completes oauth and stores token secrets on a connection", () =>
    Effect.gen(function* () {
      ...
      const started = yield* relay.oauth.start({
        endpoint: handle.discoveryUrl,
        redirectUrl: "http://localhost/callback",
        connectionId,
        tokenScope: "test-scope",
      ...
      const completed = yield* relay.oauth.complete({
      ...
      const accessToken = yield* relay.connections.accessToken(
        completed.connectionId as Parameters<typeof relay.connections.accessToken>[0],
      );
      ...
      const secretIds = new Set((yield* relay.secrets.list()).map((s) => String(s.id)));
  ...
  it.effect("registers and invokes google discovery tools with oauth headers", () =>
    Effect.gen(function* () {
      ...
      const result = yield* relay.googleDiscovery.addSource({
        name: "Google Drive",
        scope: "test-scope",
      ...
      const invocation = (yield* relay.tools.invoke(
        "drive.files.get",
  ...
  it.effect("shadowed addSource does not wipe the outer-scope source", () =>
    Effect.gen(function* () {
      ...
      yield* relay.googleDiscovery.addSource({
        name: "Org Drive",
      ...
      const userView = yield* relay.googleDiscovery.getSource("shared", USER_SCOPE_STRING);
      const orgView = yield* relay.googleDiscovery.getSource("shared", ORG_SCOPE_STRING);
  ...
  it.effect("removeSource on user shadow leaves the org row intact", () =>
    Effect.gen(function* () {
      ...
      yield* relay.googleDiscovery.removeSource("shared", USER_SCOPE_STRING);
  ...
  it.effect("usagesForSecret returns refs across auth + credential rows", () =>
    Effect.gen(function* () {
      ...
      const usages = yield* relay.secrets.usages(SecretId.make("shared-secret"));
      ...
      const connUsages = yield* relay.connections.usages(connectionId);

...
const buildRelay = (baseUrl: string) =>
  Effect.gen(function* () {
...
const buildRelayForOpenApiServer = (server: OpenApiTestServerShape) =>
  Effect.gen(function* () {
    const relay = yield* createRelay(
      makeTestConfig({
        plugins: [
...
describe("OpenAPI upstream failure modes", () => {
  ...
  it.effect("upstream 500 surfaces via the error envelope (not silent success)", () =>
    Effect.gen(function* () {
      ...
      const relay = yield* buildRelayForOpenApiServer(server);

      const exit = yield* relay.tools
  ...
  it.effect("upstream connection drop mid-response surfaces as a failure", () =>
    Effect.gen(function* () {
      ...
      const relay = yield* buildRelay(baseUrl);
  ...
  it.effect("upstream slow-then-respond doesn't lose the request", () =>
    Effect.gen(function* () {
      ...
      const result = unwrapInvocation(
        yield* relay.tools.invoke("f.things.listThings", {}, autoApprove),
      );

...
const clampedStringify = (value: unknown): string => {
  let s: string;
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: JSON.stringify may throw on cycles; fall back to String() so the upstream body can still be surfaced as ToolError.details fallback text
  try {
...
export interface OpenApiSpecConfig {
  ...
  readonly specFetchCredentials?: OpenApiSpecFetchCredentialsInput;
  /**
   * Relay scope id that owns this source row. Must be one of the
   * relay's configured scopes. Typical shape: an admin adds the
...
export const openApiPlugin = definePlugin((options?: OpenApiPluginOptions) => {
  ...
  return {
    id: "openapi" as const,
    packageName: "@relay-sh/plugin-openapi",
    ...
    // typed `OpenApiParseError` — the relay only dispatches refresh
  ...
  // the api-aware factory in `@relay-sh/plugin-openapi/api`. Hosts that

...
import { makeTestConfig, memorySecretsPlugin } from "@relay-sh/sdk/testing";
import type { ConfigFileSink } from "@relay-sh/config";
...
describe("OpenAPI Plugin", () => {
  it.effect("previewSpec returns metadata and header presets", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const preview = yield* relay.openapi.previewSpec(server.specJson);
  ...
  it.effect("registers static openapi relay tools", () =>
    Effect.gen(function* () {
      ...
      expect(ids).toContain("relay.openapi.previewSpec");
      expect(ids).toContain("relay.openapi.addSource");
    }),
  );

  it.effect("lists relay as the static runtime source", () =>
    Effect.gen(function* () {
      const clientLayer = FetchHttpClient.layer;
      ...
      const control = sources.find((s) => s.id === "relay");
  ...
  it.effect("invokes static previewSpec through relay.tools.invoke", () =>
    Effect.gen(function* () {
      const clientLayer = FetchHttpClient.layer;
  ...
  it.effect("describes static addSource parameters from Standard Schema", () =>
    Effect.gen(function* () {
      ...
      const schema = yield* relay.tools.schema("relay.openapi.addSource");
  ...
  it.effect("invokes static addSource through relay.tools.invoke", () =>
    Effect.gen(function* () {
      const clientLayer = FetchHttpClient.layer;
  ...
  it.effect("requires approval before adding a source through the runtime tool", () =>
    Effect.gen(function* () {
      ...
      expect(yield* relay.openapi.getSource("runtime_declined", TEST_SCOPE)).toBeNull();
      expect((yield* relay.tools.list()).map((t) => t.id)).not.toContain(
  ...
  it.effect("adds an org source whose direct credentials are owned by the user scope", () =>
    Effect.gen(function* () {
      ...
      yield* relay.openapi.addSpec(input);
  ...
  it.effect("updateSource removes bindings for credential slots no longer present", () =>
    Effect.gen(function* () {
      ...
      yield* relay.openapi.updateSource("stale_binding", TEST_SCOPE, {
        headers: {},
      });

      const bindings = yield* relay.openapi.listSourceBindings("stale_binding", TEST_SCOPE);
  ...
  it.effect("updateSource removes stale OAuth2 bindings when the OAuth template changes", () =>
    Effect.gen(function* () {
      ...
      yield* relay.openapi.setSourceBinding(
      ...
      yield* relay.openapi.updateSource("stale_oauth", TEST_SCOPE, {
      ...
      const bindings = yield* relay.openapi.listSourceBindings("stale_oauth", TEST_SCOPE);
  ...
  it.effect("resolves secret-backed headers at invocation time", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        yield* addOpenApiTestSource(relay, server, {
          scope: TEST_SCOPE,
          namespace: "authed",
          credentialTargetScope: TEST_SCOPE,
          headers: {
        ...
        const result = unwrapInvocation(
          yield* relay.tools.invoke("authed.items.echoHeaders", {}, autoApprove),
        );
  ...
  it.effect("addSpec without credentialTargetScope defaults to the source's scope", () =>
    ...
    // header secret in relay.jsonc errored with
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const error = yield* Effect.flip(
          relay.tools.invoke("noauth.items.listItems", {}, autoApprove),
        );
  ...
  it.effect("registers tools from an OpenAPI spec", () =>
    Effect.gen(function* () {
      ...
      const result = yield* relay.openapi.addSpec(
        testApiSourceConfig({
          namespace: "test",
          baseUrl: "",
        }),
  ...
  it.effect("invokes listItems", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const result = unwrapInvocation(
          yield* relay.tools.invoke("test.items.listItems", {}, autoApprove),
        );
        expect(result.error).toBeNull();
  ...
  it.effect("invokes getItem with path parameter", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const result = unwrapInvocation(
          yield* relay.tools.invoke("test.items.getItem", { itemId: "2" }, autoApprove),
        );
        expect(result.error).toBeNull();
  ...
  it.effect("removeSpec cleans up registered tools", () =>
    Effect.gen(function* () {
      ...
      expect((yield* relay.tools.list()).length).toBeGreaterThan(2);

      yield* relay.openapi.removeSpec("removable", TEST_SCOPE);

      const remaining = yield* relay.tools.list();
      const ids = remaining.map((t) => t.id).sort();
      expect(ids).toEqual(["relay.openapi.addSource", "relay.openapi.previewSpec"]);
    }),
  );

  it.effect("relay.sources.remove writes back to configFile (engine-level remove)", () =>
    Effect.gen(function* () {
      const clientLayer = FetchHttpClient.layer;
  ...
  it.effect("listSourceBindings returns [] for a removed source", () =>
    ...
    Effect.gen(function* () {
      ...
      const bindings = yield* relay.openapi.listSourceBindings("removable", TEST_SCOPE);
      expect(bindings).toEqual([]);
    }),
  );
  ...
  it.effect("shadowed addSpec does not wipe the outer-scope source", () =>
    Effect.gen(function* () {
      ...
      const userView = yield* relay.openapi.getSource("shared", String(USER_SCOPE));
      const orgView = yield* relay.openapi.getSource("shared", String(ORG_SCOPE));
  ...
  it.effect("removeSpec on user shadow leaves the org row intact", () =>
    Effect.gen(function* () {
      ...
      yield* relay.openapi.removeSpec("shared", String(USER_SCOPE));
  ...
  it.effect("updateSource on user shadow cannot override the inherited base URL", () =>
    Effect.gen(function* () {
      ...
      const updateResult = yield* relay.openapi
  ...
  it.effect("addSpec on user shadow cannot override the inherited base URL", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const addResult = yield* relay.openapi
          .addSpec({
            spec: server.specJson,
            scope: String(USER_SCOPE),
            namespace: "shadow_auth",
    ...
    () =>
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const relay = yield* createRelay(config);
          const db = config.db;
  ...
  it.effect("addSpec persists OAuth2 source slots with no live connection yet", () =>
    Effect.gen(function* () {
      ...
      const stored = yield* relay.openapi.getSource("deferred", TEST_SCOPE);
      ...
      const clientIdBinding = yield* relay.openapi
      ...
      const connectionBinding = yield* relay.openapi
        .listSourceBindings("deferred", TEST_SCOPE)
        .pipe(
          Effect.map(
            (bindings) =>
  ...
  it.effect("usagesForSecret aggregates header and query-param slot bindings", () =>
    Effect.gen(function* () {
      ...
      const usages = yield* relay.secrets.usages(SecretId.make("api-key"));
  ...
  it.effect("secrets.remove refuses while an openapi binding still uses it", () =>
    Effect.gen(function* () {
      ...
      const failure = yield* relay.secrets
        .remove(
      ...
      yield* relay.openapi.removeSourceBinding(
        "ref",

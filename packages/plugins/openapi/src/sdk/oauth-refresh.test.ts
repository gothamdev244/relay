...
import { makeTestConfig, serveOAuthTestServer } from "@relay-sh/sdk/testing";
import {
  addOpenApiTestSource,
  serveOpenApiHttpApiTestServer,
  unwrapInvocation,
} from "@relay-sh/plugin-openapi/testing";
...
// ---------------------------------------------------------------------------
// Fixture builder. Wires up a single-scope relay with an in-memory
...
const makeRelay = () =>
  Effect.gen(function* () {
    ...
    // Seed client id + secret in the relay scope so the openapi
    ...
    return { relay, scopeId, openApiServer };
  });

type EffectSuccess<T> = T extends Effect.Effect<infer A, unknown, unknown> ? A : never;

type RelayValue = EffectSuccess<ReturnType<typeof makeRelay>>["relay"];
...
const bindOAuthConnection = (
  ...
  relay.openapi.setSourceBinding(
...
describe("OpenAPI oauth refresh", () => {
  it.effect("expired access_token is refreshed via grant_type=refresh_token before invoke", () =>
    Effect.gen(function* () {
      const { relay, scopeId, openApiServer } = yield* makeRelay();
      ...
      yield* addOpenApiTestSource(relay, openApiServer, {
        scope: String(scopeId),
        namespace: "petstore",
        oauth2: auth,
      });
      yield* bindOAuthConnection(relay, scopeId, "conn-refresh-ok", auth);

      const result = unwrapInvocation(
        yield* relay.tools.invoke("petstore.items.echoHeaders", {}, autoApprove),
      );
      ...
      const conn = yield* relay.connections.get("conn-refresh-ok");
  ...
  it.effect("concurrent invokes with an expired token issue exactly one refresh", () =>
    Effect.gen(function* () {
      ...
      yield* bindOAuthConnection(relay, scopeId, "conn-refresh-concurrent", auth);

      const invokes = yield* Effect.all(
        [1, 2, 3, 4, 5].map(() =>
          relay.tools.invoke("petstore.items.echoHeaders", {}, autoApprove),
        ),
  ...
  it.effect("invalid_grant from refresh surfaces as ConnectionReauthRequiredError", () =>
    Effect.gen(function* () {
      ...
      yield* bindOAuthConnection(relay, scopeId, "conn-refresh-dead", auth);
      ...
      const flipped = yield* relay.connections.accessToken("conn-refresh-dead").pipe(
        Effect.flip,

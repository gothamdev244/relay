...
import { ConnectionId, ScopeId, SecretId } from "@relay-sh/sdk/shared";
import { startOAuth } from "@relay-sh/react/api/atoms";
import { useScope, useScopeStack } from "@relay-sh/react/api/scope-context";
import { connectionWriteKeys, sourceWriteKeys } from "@relay-sh/react/api/reactivity-keys";
...
import { HeadersList } from "@relay-sh/react/plugins/headers-list";
import {
...
} from "@relay-sh/react/plugins/oauth-sign-in";
import {
  CreatableSecretPicker,
  matchPresetKey,
  type HeaderState,
} from "@relay-sh/react/plugins/secret-header-auth";
import { CredentialScopeDropdown } from "@relay-sh/react/plugins/credential-target-scope";
import { slugifyNamespace, useSourceIdentity } from "@relay-sh/react/plugins/source-identity";
import { useSecretPickerSecrets } from "@relay-sh/react/plugins/use-secret-picker-secrets";
import { Button } from "@relay-sh/react/components/button";
import { CopyButton } from "@relay-sh/react/components/copy-button";
import {
  Collapsible,
  CollapsibleContent,
  CollapsibleTrigger,
} from "@relay-sh/react/components/collapsible";
import {
  CardStack,
  CardStackContent,
  CardStackEntryField,
} from "@relay-sh/react/components/card-stack";
import { FieldLabel } from "@relay-sh/react/components/field";
import { FloatActions } from "@relay-sh/react/components/float-actions";
import { HelpTooltip } from "@relay-sh/react/components/help-tooltip";
import { Label } from "@relay-sh/react/components/label";
import { Textarea } from "@relay-sh/react/components/textarea";
import { Checkbox } from "@relay-sh/react/components/checkbox";
import { RadioGroup, RadioGroupItem } from "@relay-sh/react/components/radio-group";
import { IOSSpinner, Spinner } from "@relay-sh/react/components/spinner";
...
export function resolveOAuthUrl(url: string, baseUrl: string): string {
  if (!url) return url;
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: URL constructor normalizes provider metadata URLs
  try {
    new URL(url);
    return url;
  } catch {
    if (!baseUrl) return url;
    // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: URL constructor resolves relative provider metadata URLs
    try {
  ...
  const handleConnectOAuth2 = useCallback(async () => {
    ...
    await oauth.openAuthorization({
      ...
      run: async () => {
        ...
        if (Exit.isFailure(exit)) {
          // oxlint-disable-next-line relay/no-try-catch-or-throw, relay/no-error-constructor -- boundary: OAuth popup API represents start failure by rejecting run()

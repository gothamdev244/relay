...
import { OAuthTestServer, serveTestHttpServerLayer } from "@relay-sh/sdk/testing";
import { isToolResult, type ScopeId } from "@relay-sh/sdk/core";
...
export type OpenApiTestSourceRelay = {
...
export const addOpenApiTestSource = (
  relay: OpenApiTestSourceRelay,
  server: OpenApiTestServerShape,
  options: OpenApiTestSourceOptions,
) => relay.openapi.addSpec(makeOpenApiTestSourceConfig(server, options));
...
export const addOpenApiHttpApiTestSource = (
  ...
  api: HttpApi.Any,
  options: OpenApiHttpApiTestSourceOptions,
) => relay.openapi.addSpec(makeOpenApiHttpApiTestSourceConfig(api, options));
...
const OpenApiEchoApi = HttpApi.make("executorOpenApiTest")
  ...
  .annotateMerge(
    OpenApi.annotations({
      title: "Relay OpenAPI Test Server",
      version: "1.0.0",
    }),
  );
...
export class OpenApiHttpApiTestServer extends Context.Service<
  ...
  OpenApiHttpApiTestServerShape
>()("@relay-sh/plugin-openapi/testing/OpenApiHttpApiTestServer") {
...
export class OpenApiEchoTestServer extends Context.Service<
  ...
  OpenApiEchoTestServerShape
>()("@relay-sh/plugin-openapi/testing/OpenApiEchoTestServer") {

...
// `relay.config.ts` or `relay.jsonc` invalidates it and triggers
...
import type { RelayCliConfig } from "@relay-sh/sdk";

const VIRTUAL_ID = "virtual:relay/plugins-client";
...
const DEFAULT_JSONC_CANDIDATES = ["relay.jsonc", "relay.json"];
...
interface RelayVitePluginOptions {
  /**
   * Path to the relay config file. Resolved relative to the Vite
   * project root if not absolute. Defaults to the first match of
   * `relay.config.ts` / `.js` / `.mjs` (with a fallback under
   * `src/`).
   */
  readonly configPath?: string;
  /**
   * Path to the relay jsonc manifest. Resolved relative to the Vite
   * project root if not absolute. Defaults to `relay.jsonc`.
   */
  readonly jsoncPath?: string;
}

export default function relayVitePlugin(options: RelayVitePluginOptions = {}): Plugin {
  ...
  const loadVirtualSource = async (): Promise<string> => {
    ...
    for (const entry of entries) {
      ...
      if (!resolved) {
        ...
        console.warn(
          `[@relay-sh/vite-plugin] plugin package "${entry.pkg}" listed but ` +
      ...
      const ident = `__relay_plugin_${exportExpressions.length}`;
      ...
      // Resolving here (from the consumer's relay.config dir) and
...
// Consumers wanting strong typing for `virtual:relay/plugins-client`

...
// Worker filters on `agent.includes("relay")`. Channel/version/client are
...
}): string => `relay/${input.channel}/${input.version}/${input.client}`;
...
>()("@relay-sh/integrations-registry/IntegrationsRegistry") {}
...
export interface IntegrationsRegistryConfig {
  ...
  /** Override the cache directory. Defaults to $RELAY_DATA_DIR/cache or ~/.relay/cache. */
...
const resolveCacheDir = (override: string | undefined): string => {
  ...
  const dataDir = process.env.RELAY_DATA_DIR ?? pathJoin(homedir(), ".relay");

...
//
// Reads `relay.jsonc#plugins`, dynamically imports each package's
...
import type { AnyPlugin } from "@relay-sh/sdk";
...
import { RelayFileConfig } from "./schema";
...
export interface LoadPluginsFromJsoncOptions {
  /** Absolute path to `relay.jsonc` (or compatible). */
 ...
 * the static `relay.config.ts` factory."
 */
...
const loadPluginsFromJsoncEffect = (
  ...
  Effect.gen(function* () {
    ...
    const config = yield* Schema.decodeUnknownEffect(RelayFileConfig)(parsed).pipe(
      Effect.mapError(
        (error) =>
    ...
    // (relevant when the dev server kicks a reload after `relay plugin
    // install`).

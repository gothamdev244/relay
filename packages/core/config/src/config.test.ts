...
const decodeRelayFileConfig = Schema.decodeUnknownSync(RelayFileConfig);

const withTmpDir = <A, E>(fn: (dir: string) => Effect.Effect<A, E, FileSystem.FileSystem>) =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const dir = yield* fs.makeTempDirectoryScoped({
      prefix: "relay-config-test-",
    });
    return yield* fn(dir);
  }).pipe(Effect.scoped, Effect.provide(NodeFileSystem.layer));

describe("RelayFileConfig schema", () => {
  it("decodes a minimal config", () => {
    const raw = { sources: [] };
    const result = decodeRelayFileConfig(raw);
  ...
  it("rejects invalid source kind", () => {
    ...
    expect(() => decodeRelayFileConfig(raw)).toThrow();
  });
});
...
describe("loadConfig", () => {
  ...
  it.effect("loads a valid JSONC file with comments", () =>
    withTmpDir((dir) =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = join(dir, "relay.jsonc");

...
const handleLogin = async (request: Request) => {
  ...
  if (!basic || basic.username !== "alice" || basic.password !== "password") {
    return json(
      { error: "access_denied" },
      { status: 401, headers: { "www-authenticate": 'Basic realm="Relay test servers"' } },
    );
  }
...
const OpenApiEchoApi = HttpApi.make("relayOpenApiWorkerTest")
  .add(OpenApiEchoItemsGroup)
  .annotateMerge(
    OpenApi.annotations({ title: "Relay Worker OpenAPI Test Server", version: "1.0.0" }),
  );
...
const createMcpServer = () => {
  const server = new McpServer(
    { name: "relay-worker-mcp-test", version: "1.0.0" },
    { capabilities: {} },
  );
  server.registerTool(
    "hello",

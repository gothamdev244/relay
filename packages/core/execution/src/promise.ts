// ---------------------------------------------------------------------------
// @relay-sh/execution/promise — Promise-native surface for the execution
// engine.
// ---------------------------------------------------------------------------
//
...
// from `@relay-sh/execution` to keep trace context intact.
// ---------------------------------------------------------------------------
...
import type {
  ElicitationContext,
  ElicitationResponse,
  Relay as EffectRelay,
} from "@relay-sh/sdk/core";
import { ToolId } from "@relay-sh/sdk/core";
import type { Relay as PromiseRelay } from "@relay-sh/sdk/promise";
import type { CodeExecutionError, CodeRelay, ExecuteResult } from "@relay-sh/codemode-core";
...
/**
 * Wrap a Promise-style relay into the Effect shape the engine consumes.
 */
const fromPromise = <A>(try_: () => Promise<A>): Effect.Effect<A> =>
  // oxlint-disable-next-line relay/no-effect-escape-hatch -- boundary: Promise relay facade has already erased the SDK typed error channel
  Effect.tryPromise({ try: try_, catch: (cause) => cause }).pipe(Effect.orDie);

type EffectInvokeOptions = Parameters<EffectRelay["tools"]["invoke"]>[2];
type PromiseInvokeOptions = Parameters<PromiseRelay["tools"]["invoke"]>[2];
...
const wrapPromiseRelay = (pe: PromiseRelay): EffectRelay => ({
  scopes: pe.scopes,
  tools: {
...
export const toPromiseExecutionEngine = <E extends Cause.YieldableError>(
  ...
  execute: (code, options) =>
    Effect.runPromise(
      engine.execute(code, {
        onElicitation: (ctx) =>
          // oxlint-disable-next-line relay/no-effect-escape-hatch -- boundary: host-provided Promise elicitation callback is outside the Effect error model
...
export const createExecutionEngine = <E extends Cause.YieldableError = CodeExecutionError>(
  ...
  toPromiseExecutionEngine(
    createEffectExecutionEngine({
      relay: wrapPromiseRelay(config.relay),
      codeRelay: config.codeRelay,
    }),
  );

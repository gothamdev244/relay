...
import type { Relay, Source } from "@relay-sh/sdk/core";
 ...
 */
export const buildExecuteDescription = (relay: Relay): Effect.Effect<string> =>
  Effect.gen(function* () {
    const sources: readonly Source[] = yield* relay.sources.list().pipe(
      // oxlint-disable-next-line relay/no-effect-escape-hatch -- boundary: ExecutionEngine.getDescription currently exposes no error channel; engine typed-error widening is covered separately
      Effect.orDie,
      Effect.withSpan("relay.sources.list"),
    );

    const description = yield* Effect.sync(() => formatDescription(sources)).pipe(
      Effect.withSpan("schema.compile.description", {
        attributes: { "relay.source_count": sources.length },
      }),
    );

    yield* Effect.annotateCurrentSpan({
      "relay.source_count": sources.length,
      "schema.kind": "execute",
    });
...
const formatDescription = (sources: readonly Source[]): string => {
  const lines: string[] = [
    ...
    "5. Use `tools.relay.sources.list()` when you need configured source inventory.",
    ...
    "- `tools.relay.sources.list()` returns the same paged shape: `{ items: [{ id, toolCount, ... }], total, hasMore, nextOffset }`.",
    ...
    "- If `hasMore` is true and you didn't find what you need, fetch the next page: `tools.search({ query, offset: nextOffset, limit })`. Same `offset` parameter on `tools.relay.sources.list({ offset, limit })`.",
    ...
    "- The `tools` object is a lazy proxy — `Object.keys(tools)` won't work. Use `tools.search()` or `tools.relay.sources.list()` instead.",
    '- Pass an object to system tools, e.g. `tools.search({ query: "..." })`, `tools.relay.sources.list()`, and `tools.describe.tool({ path })`.',

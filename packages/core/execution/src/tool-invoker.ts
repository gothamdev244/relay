 ...
 * attribute is always populated without hitting `relay.sources.list()`
 * per call.
 */
 ...
 * `relay.tools.invoke(toolId, args)`.
 *
 ...
 */
export const makeRelayToolInvoker = (
  relay: Relay,
  ...
  invoke: Effect.fn("mcp.tool.dispatch")(function* ({ path, args }) {
    ...
    const result = yield* relay.tools.invoke(path as ToolId, args, options.invokeOptions).pipe(
      Effect.catchCause((cause): Effect.Effect<never, ExecutionToolError> => {
        ...
        return Effect.logError("tool dispatch failed", cause).pipe(
          Effect.annotateLogs({
            "relay.correlation_id": correlationId,
            "mcp.tool.name": path,
          }),
          Effect.flatMap(() =>
            Effect.fail(
...
export type RelaySourceListItem = {
  readonly id: string;
  readonly name: string;
 ...
 * `tools.search` and `tools.relay.sources.list` so the model sees one
 * consistent shape:
 *
...
export const searchTools = Effect.fn("relay.tools.search")(function* (
  relay: Relay,
  query: string,
  limit = 12,
  ...
  yield* Effect.annotateCurrentSpan({
    "relay.search.query_length": query.length,
    "relay.search.limit": limit,
    "relay.search.offset": offset,
    ...(options?.namespace ? { "relay.search.namespace": options.namespace } : {}),
  });
  ...
  const all = yield* relay.tools.list({ includeAnnotations: false }).pipe(
    Effect.mapError(
      (cause) =>
        new ExecutionToolError({
  ...
  yield* Effect.annotateCurrentSpan({
    "relay.search.candidate_count": all.length,
    "relay.search.match_count": ranked.length,
    "relay.search.result_count": page.items.length,
    "relay.search.has_more": page.hasMore,
  });
  return page;
});

/** What `tools.relay.sources.list()` calls inside the sandbox. */
export const listRelaySources = Effect.fn("relay.sources.list")(function* (
  relay: Relay,
  options?: {
  ...
  const offset = options?.offset ?? 0;
  const sources = yield* relay.sources.list().pipe(
    Effect.mapError(
      (cause) =>
        new ExecutionToolError({
          message: "Failed to list relay sources",
          cause,
        }),
    ),
  );
  ...
  const allTools = yield* relay.tools.list({ includeAnnotations: false }).pipe(
    Effect.mapError(
      (cause) =>
        new ExecutionToolError({
  ...
  yield* Effect.annotateCurrentSpan({
    "relay.sources.candidate_count": sources.length,
    "relay.sources.match_count": sortedWithCounts.length,
    "relay.sources.result_count": page.items.length,
    "relay.sources.has_more": page.hasMore,
  });
  return page;
});

/** What `tools.describe.tool()` calls inside the sandbox. */
export const describeTool = Effect.fn("relay.tools.describe")(function* (
  relay: Relay,
  path: string,
) {
  ...
  const schema: ToolSchema | null = yield* relay.tools.schema(path);

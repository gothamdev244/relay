...
import { ElicitationResponse, createRelay, definePlugin } from "@relay-sh/sdk";
import { makeTestConfig } from "@relay-sh/sdk/testing";
import { ExecutionToolError } from "./errors";
import { makeRelayToolInvoker } from "./tool-invoker";
      ...
      tools: [
        ...
        },
        {
          ...
          handler: () =>
            Effect.fail(
              Object.assign(
                // oxlint-disable-next-line relay/no-error-constructor -- boundary: leak test deliberately fails with a raw Error + crafted stack to assert the dispatcher's opaque-generic redaction
...
describe("internal-error leak audit (opaque defects)", () => {
  it.effect("plugin tagged error: defect surfaces only as opaque generic + correlation id", () =>
    Effect.gen(function* () {
      const relay = yield* createRelay(makeTestConfig({ plugins: [leakyPlugin()] as const }));
      ...
      // oxlint-disable-next-line relay/no-unknown-error-message -- boundary: leak test inspects the rendered message to assert it is the opaque generic

...
import { describeTool, makeRelayToolInvoker, searchTools } from "./tool-invoker";

const codeRelay = makeQuickJsRelay();
...
const makeSearchRelay = () =>
  createRelay(makeTestConfig({ plugins: [githubPlugin(), crmPlugin()] as const }));

describe("tool discovery", () => {
  it.effect("ranks matches using ids, namespaces, camelCase names, and descriptions", () =>
    Effect.gen(function* () {
      const relay = yield* makeSearchRelay();

      const githubMatches = yield* searchTools(relay, "github issues", 5);
      ...
      const repoMatches = yield* searchTools(relay, "repo details", 5);
      expect(repoMatches.items[0]?.path).toBe("github.getRepositoryDetails");

      const crmMatches = yield* searchTools(relay, "crm create contact", 5);
  ...
  it.effect("returns no matches for empty queries instead of listing arbitrary tools", () =>
    Effect.gen(function* () {
      const relay = yield* makeSearchRelay();
      const matches = yield* searchTools(relay, "", 5);
  ...
  it.effect("paginates ranked matches via limit + offset with hasMore + nextOffset", () =>
    Effect.gen(function* () {
      ...
      const all = yield* searchTools(relay, "list", 100);
      ...
      const firstPage = yield* searchTools(relay, "list", 1);
      ...
      const secondPage = yield* searchTools(relay, "list", 1, {
      ...
      const past = yield* searchTools(relay, "list", 5, { offset: all.total + 10 });
  ...
  it.effect("can narrow discovery to a namespace", () =>
    Effect.gen(function* () {
      ...
      const githubOnly = yield* searchTools(relay, "list", 5, {
        namespace: "github",
      });
      expect(githubOnly.items.map((match) => match.path)).toEqual(["github.listRepositoryIssues"]);

      const crmOnly = yield* searchTools(relay, "list", 5, {
        namespace: "crm",
      });
      expect(crmOnly.items.map((match) => match.path)).toEqual(["crm.listContacts"]);

      const sandboxResult = yield* createExecutionEngine({ relay, codeRelay }).execute(
  ...
  it.effect("supports relay-scoped source listing and tool search", () =>
    Effect.gen(function* () {
      const relay = yield* makeSearchRelay();

      const listed = yield* createExecutionEngine({ relay, codeRelay }).execute(
        "return await tools.relay.sources.list();",
        { onElicitation: acceptAll },
      );
      ...
      const searched = yield* createExecutionEngine({ relay, codeRelay }).execute(
  ...
  it.effect("paginates source listings via limit + offset", () =>
    Effect.gen(function* () {
      ...
      const firstPage = yield* engine.execute(
        "return await tools.relay.sources.list({ limit: 1 });",
        { onElicitation: acceptAll },
      );
      ...
      const secondPage = yield* engine.execute(
        "return await tools.relay.sources.list({ limit: 1, offset: 1 });",
        { onElicitation: acceptAll },
      );
  ...
  it.effect("rejects negative offsets via the engine validator", () =>
    Effect.gen(function* () {
      ...
      const badList = yield* engine.execute(
        [
          "try {",
          "  await tools.relay.sources.list({ offset: -5 });",
          '  return "unexpected";',
          "} catch (error) {",
          "  return error instanceof Error ? error.message : String(error);",
          "}",
      ...
      expect(String(badList.result)).toContain(
        "tools.relay.sources.list offset must be a non-negative number when provided",
      );
    }),
  );
  ...
  it.effect("describes tools with TypeScript previews", () =>
    Effect.gen(function* () {
      ...
      const described = yield* describeTool(relay, "github.listRepositoryIssues");
    ...
    Effect.gen(function* () {
      ...
      const execution = yield* engine.execute(
        [
          ...
          "const result = await tools.github.getRepositoryDetails({ owner: 'relay', repo: 'relay' });",
          "return {",
  ...
  it.effect("describes the ToolResult wrapper through the direct describe helper", () =>
    Effect.gen(function* () {
      const relay = yield* makeSearchRelay();
      const described = yield* describeTool(relay, "github.getRepositoryDetails");
  ...
  it.effect("passes ToolResult.fail through to the sandbox as a value (no throw)", () =>
    Effect.gen(function* () {
      const relay = yield* createRelay(makeTestConfig({ plugins: [errorPlugin()] as const }));
      const invoker = makeRelayToolInvoker(relay, {
...
describe("pause/resume with multiple elicitations", () => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const relay = yield* makeElicitingRelay();
        const engine = createExecutionEngine({ relay, codeRelay });
  ...
  it("resume returns across separate runPromise boundaries for a single-elicit tool (HTTP-like)", async () => {
    const relay = await Effect.runPromise(makeElicitingRelay());
    const engine = createExecutionEngine({ relay, codeRelay });

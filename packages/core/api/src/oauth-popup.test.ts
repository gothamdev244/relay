...
describe("popupDocument", () => {
  ...
  it("posts to window.opener AND falls back to BroadcastChannel with the given channel name", () => {
    const html = popupDocument(successPayload, "relay:openapi-oauth-result");
    expect(html).toContain("window.opener.postMessage(p,window.location.origin)");
    expect(html).toContain('new BroadcastChannel("relay:openapi-oauth-result")');
    ...
    const html = await Effect.runPromise(
      runOAuthCallback<GoogleAuth, DomainError, never>({
        ...
        toErrorMessage: (error) => {
          ...
          // oxlint-disable-next-line relay/no-unknown-error-message -- boundary: schema guard narrows the unknown popup callback error to the public test message

...
//
// The SDK (`@relay-sh/sdk`) stays storage-typed: plugin code and
// relay surface methods return `StorageError` in their typed error
...
import type { StorageFailure } from "@relay-sh/sdk/core";
import { InternalError } from "@relay-sh/sdk/core";

// Re-export so existing `@relay-sh/api` consumers keep working.
...
export const capture = <A, E, R>(
  ...
  (eff as Effect.Effect<A, E | StorageFailure, R>).pipe(
    // oxlint-disable-next-line relay/no-effect-escape-hatch -- boundary: unique conflicts that reach the HTTP edge are unexpected defects captured by observabilityMiddleware
...
export class ObservabilityMiddleware extends HttpApiMiddleware.Service<ObservabilityMiddleware>()(
  "@relay-sh/api/ObservabilityMiddleware",
  { error: InternalError },
) {}

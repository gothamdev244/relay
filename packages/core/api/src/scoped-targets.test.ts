...
import { CoreHandlers, ExecutionEngineService, RelayService } from "./server";

const webHandlerFor = (relay: Relay) =>
  Effect.acquireRelease(
    Effect.sync(() =>
      HttpRouter.toWebHandler(
        HttpApiBuilder.layer(RelayApi).pipe(
          Layer.provide(CoreHandlers),
          Layer.provide(observabilityMiddleware(RelayApi)),
          Layer.provide(Layer.succeed(RelayService)(relay)),
          Layer.provide(
...
const handlerContextFor = (relay: Relay) =>
  Context.make(RelayService, relay).pipe(
...
describe("core API explicit target scopes", () => {
  it.effect("policy update uses the row target scope instead of the route read scope", () =>
    Effect.gen(function* () {
      ...
      const web = yield* webHandlerFor(relay);
      const context = handlerContextFor(relay);

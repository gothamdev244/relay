...
// local (`composePluginHandlers(plugins, relay)`), per-request for
// cloud (`providePluginExtensions(plugins)(relay)` in the auth
// middleware).
//
...
import type { AnyPlugin, PluginExtensions } from "@relay-sh/sdk";

import { CoreRelayApi } from "./api";
 ...
 *  relay groups to type `composePluginApi(plugins)` precisely. */
export type PluginGroups<TPlugins extends readonly AnyPlugin[]> = ExtractPluginGroup<
  TPlugins[number]
>;

/** Group identities baked into `CoreRelayApi` (tools, sources, secrets,
 *  …). Extracted via inference so adding a core group flows through
 *  without touching this helper. */
type CoreGroups =
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  typeof CoreRelayApi extends HttpApi.HttpApi<any, infer G> ? G : never;

/** Result of `composePluginApi(plugins)` — the core API extended with
 *  every plugin group from `TPlugins`. */
export type ComposedRelayApi<TPlugins extends readonly AnyPlugin[]> = HttpApi.HttpApi<
  "relay",
...
/**
 * Compose plugin-contributed `HttpApiGroup`s into the core relay API.
 * Plugins without a `routes()` field are skipped.
 *
 * Returns a precisely typed `HttpApi<"relay", CoreGroups |
...
export const composePluginApi = <TPlugins extends readonly AnyPlugin[]>(
  plugins: TPlugins,
): ComposedRelayApi<TPlugins> => {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  let api: any = CoreRelayApi;
  ...
  return api as ComposedRelayApi<TPlugins>;
};

/**
 * Build the merged Layer of plugin handler implementations, satisfying
 * each plugin's `extensionService` Tag eagerly from `relay[id]`.
 *
 * Suitable for hosts (like local) that have a single, boot-time
 * relay. Hosts with per-request relays (cloud) should use
...
export const composePluginHandlers = <TPlugins extends readonly AnyPlugin[]>(
  ...
  // oxlint-disable-next-line relay/no-double-cast
 ...
 * effect via `Effect.provideService(tag, relay[id])`. The plugin
 ...
 *   return yield* httpEffect.pipe(provide(requestRelay));
 */

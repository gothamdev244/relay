...
const isUniqueViolation = (cause: unknown): boolean => {
  ...
  for (let i = 0; i < 5; i += 1) {
    ...
    if (!err) return false;
    const code = err["code"];
    // oxlint-disable-next-line relay/no-unknown-error-message -- boundary: database drivers expose unique-violation details on native error messages
...
const causeMessage = (cause: unknown): string | undefined => {
  const message =
    cause && typeof cause === "object"
      ? // oxlint-disable-next-line relay/no-unknown-error-message -- boundary: preserve database driver error text inside typed StorageError
...
export const activeFumaDbRef = Context.Reference<FumaDb | null>("relay/ActiveFumaDb", {
  defaultValue: () => null,
});
...
const assertAllowedTable = (tables: ReadonlySet<string> | undefined, table: PropertyKey): void => {
  ...
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: plugin-facing FumaDB facade rejects unavailable tables synchronously before query execution
      ...
      return Effect.tryPromise({
        try: () =>
          db.transaction(async (transactionDb) => {
            ...
            // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: FumaDB transactions roll back when the callback rejects
...
export class FumaClient extends Context.Service<FumaClient, IFumaClient>()("relay/FumaClient") {

...
import { scopedRelayTable, type ToolPolicyRow } from "./core-schema";
...
import { makeTestRelay } from "./testing";

// ---------------------------------------------------------------------------
// Pure unit tests — pattern matcher + resolution. No relay required.
// ---------------------------------------------------------------------------
...
describe("resolveToolPolicy", () => {
  ...
  it("does not allow an inner approve to weaken an outer block", () => {
    // Scope rank comes from the relay's scope stack: 0 = innermost,
...
// ---------------------------------------------------------------------------
// Relay integration — exercises invoke + list + CRUD with FumaDB/SQLite
...
const policyTestSchema = {
  ptest_marker: scopedRelayTable("ptest_marker", {}),
};
...
const setupRelay = () =>
  makeTestRelay({ plugins: [policyTestPlugin()] as const }).pipe(
    Effect.tap((relay) => relay.ptest.seed()),
  );

describe("relay.policies", () => {
  it.effect("list is empty when no rules exist", () =>
    Effect.gen(function* () {
      const relay = yield* setupRelay();
      const rules = yield* relay.policies.list();
  ...
  it.effect("create defaults new rules to the top of the list", () =>
    Effect.gen(function* () {
      const relay = yield* setupRelay();
      const first = yield* relay.policies.create({
        targetScope: "test-scope",
        pattern: "vercel.*",
        action: "require_approval",
      });
      const second = yield* relay.policies.create({
        targetScope: "test-scope",
  ...
  it.effect("create stores rules at the requested target scope in a stacked context", () =>
    Effect.gen(function* () {
      ...
      const relay = yield* makeTestRelay({
        scopes: [
  ...
  it.effect("update with a new position reorders the list", () =>
    Effect.gen(function* () {
      const relay = yield* setupRelay();
      const a = yield* relay.policies.create({
        targetScope: "test-scope",
        pattern: "a.*",
        action: "approve",
      });
      const b = yield* relay.policies.create({
        targetScope: "test-scope",
        pattern: "b.*",
        action: "approve",
      });
      // After two creates: b above a (newer = higher precedence).
      const before = yield* relay.policies.list();
      ...
      const after = yield* relay.policies.list();
  ...
  it.effect("consecutive creates produce strictly increasing-precedence keys", () =>
    Effect.gen(function* () {
      ...
      for (const pattern of ["a.*", "b.*", "c.*", "d.*"]) {
        const row = yield* relay.policies.create({
          targetScope: "test-scope",
          pattern,
          action: "approve",
        });
  ...
  it.effect("remove deletes the row", () =>
    Effect.gen(function* () {
      ...
      yield* relay.policies.remove({ id: created.id, targetScope: "test-scope" });
      const rules = yield* relay.policies.list();
      expect(rules).toEqual([]);
    }),
  );
  ...
  it.effect("resolve returns the matching rule with provenance", () =>
    Effect.gen(function* () {
      ...
      const result = yield* relay.policies.resolve("vercel.deploy");
      expect(result?.action).toBe("block");
...
describe("blocked tools", () => {
  ...
  it.effect("are visible when includeBlocked is true", () =>
    Effect.gen(function* () {
      ...
      const tools = yield* relay.tools.list({ includeBlocked: true });
      const ids = tools.map((t) => t.id).sort();
  ...
  it.effect("invoke fails with ToolBlockedError carrying the matched pattern", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.result(relay.tools.invoke("vercel.delete", {}));
      expect(Result.isFailure(result)).toBe(true);
  ...
  it.effect("outer block still applies when an inner scope approves", () =>
    Effect.gen(function* () {
      ...
      yield* relay.ptest.seed(String(orgScope));
      ...
      const policy = yield* relay.policies.resolve("vercel.deploy");
      expect(policy?.action).toBe("block");

      const tools = yield* relay.tools.list();
      expect(tools.map((tool) => tool.id)).not.toContain("vercel.deploy");

      const result = yield* Effect.result(relay.tools.invoke("vercel.deploy", {}));
      expect(Result.isFailure(result)).toBe(true);
      if (!Result.isFailure(result)) return;
      expect(Predicate.isTagged("ToolBlockedError")(result.failure)).toBe(true);
    }),

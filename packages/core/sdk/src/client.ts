// ---------------------------------------------------------------------------
// @relay-sh/sdk/client — frontend half of the plugin SDK.
//
...
// existing `RelayApiClient` (see packages/react/src/api/client.tsx).
...
// ---------------------------------------------------------------------------
// RelayPluginsProvider + hooks — host-level distribution of the loaded
// `ClientPluginSpec[]` via React context.
//
// The host wraps once at the root of its tree (typically reading from
// `virtual:relay/plugins-client`); pages and shared components consume
...
interface RelayPluginsContextValue {
...
const RelayPluginsContext = createContext<RelayPluginsContextValue | null>(null);
RelayPluginsContext.displayName = "RelayPluginsContext";

export interface RelayPluginsProviderProps {
  readonly plugins: readonly ClientPluginSpec[];
  readonly children: ReactNode;
}

export function RelayPluginsProvider(
  props: RelayPluginsProviderProps,
): ReturnType<typeof createElement> {
  const { plugins, children } = props;
  const value = useMemo<RelayPluginsContextValue>(
    () => ({
      plugins,
  ...
  return createElement(RelayPluginsContext.Provider, { value }, children);
}

const usePluginsCtx = (hookName: string): RelayPluginsContextValue => {
  const ctx = useContext(RelayPluginsContext);
  if (!ctx) {
    // oxlint-disable-next-line relay/no-try-catch-or-throw, relay/no-error-constructor -- boundary: React hook invariant
    throw new Error(`${hookName} must be called inside an <RelayPluginsProvider>.`);
  }
  return ctx;
};

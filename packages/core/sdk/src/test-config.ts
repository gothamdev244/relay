...
import { collectTables, createRelay, type Relay, type RelayConfig } from "./relay";
import type { FumaDb } from "./fuma-runtime";
import { ScopeId } from "./ids";
import { definePlugin, type AnyPlugin } from "./plugin";
import { Scope } from "./scope";
import type { RelayScopePolicyContext } from "./scope-policy";
import type { SecretProvider } from "./secrets";
import type { SqliteTestFumaDb } from "./sqlite-test-db";

// ---------------------------------------------------------------------------
// makeTestConfig — build an RelayConfig backed by an in-memory FumaDB.
...
const makeLazyTestFumaDb = (options: {
  ...
  const makeDb = (context?: RelayScopePolicyContext): FumaDb =>
    new Proxy(
      { internal: context === undefined ? internal : { ...internal, context } },
      {
        get(target, prop) {
          ...
          if (prop === "withContext") {
            return (nextContext: RelayScopePolicyContext) => makeDb(nextContext);
          }
...
> {
  readonly config: RelayConfig<TPlugins> & { readonly testDb: TestFumaDb };
  readonly relay: Relay<TPlugins>;
  readonly testDb: TestFumaDb;
  readonly scopes: readonly Scope[];
}

export class TestWorkspace extends Context.Service<TestWorkspace, TestWorkspaceHarness>()(
  "relay-sdk/TestWorkspace",
) {
...
export const makeTestRelay = <const TPlugins extends readonly AnyPlugin[] = readonly []>(
  options?: TestConfigOptions<TPlugins>,
) => makeTestWorkspaceHarness(options).pipe(Effect.map(({ relay }) => relay));

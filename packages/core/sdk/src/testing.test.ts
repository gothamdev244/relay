...
layer(TestLayer, { timeout: "15 seconds" })("testing fixtures", (it) => {
  it.effect("TestWorkspace exposes the real relay with an explicit scope stack", () =>
    Effect.gen(function* () {
      ...
      expect(workspace.relay.scopes.map((scope) => String(scope.id))).toEqual(["test-scope"]);
      expect(yield* workspace.relay.secrets.providers()).toEqual(["memory"]);
    }),
  );
  ...
  it.effect("OAuthTestServer completes a real authorization-code OAuth flow", () =>
    Effect.gen(function* () {
      ...
      yield* workspace.relay.secrets.set(
        SetSecretInput.make({
      ...
      const started = yield* workspace.relay.oauth.start({
      ...
      const completed = yield* workspace.relay.oauth.complete({
      ...
      const accessToken = yield* workspace.relay.connections.accessToken(completed.connectionId);
  ...
  it.effect("OAuthTestServer supports MCP-style dynamic client registration", () =>
    Effect.gen(function* () {
      ...
      const probe = yield* workspace.relay.oauth.probe({ endpoint: oauth.mcpResourceUrl });

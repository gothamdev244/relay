...
const setSecret = (relay: Relay, scopeId: ScopeId, id: string, value: string) =>
  relay.secrets.set(
    SetSecretInput.make({
      id: SecretId.make(id),
      scope: scopeId,
...
const createConnection = (relay: Relay, scopeId: ScopeId, id: string) =>
...
describe("credential bindings", () => {
  it.effect("resolves a user-workspace credential for an inherited org source", () =>
    Effect.gen(function* () {
      ...
      const resolved = yield* userRelay.credentialBindings.resolve({
        pluginId: TEST_PLUGIN_ID,
        sourceId: TEST_SOURCE_ID,
        sourceScope: harness.scopes.org.id,
        slotKey: TEST_SLOT,
  ...
  it.effect("workspace credential bindings shadow org bindings without copying the source", () =>
    Effect.gen(function* () {
      ...
      const orgRelay = yield* harness.create([harness.scopes.org]);
      yield* orgRelay.credentialTest.registerSource(harness.scopes.org.id);
      yield* setSecret(orgRelay, harness.scopes.org.id, "api-token", "sk-org");
      yield* orgRelay.credentialBindings.set({
      ...
      const resolved = yield* workspaceRelay.credentialBindings.resolve({
        pluginId: TEST_PLUGIN_ID,
        sourceId: TEST_SOURCE_ID,
        sourceScope: harness.scopes.org.id,
        slotKey: TEST_SLOT,
      });
      const sources = yield* workspaceRelay.sources.list();
  ...
  it.effect("rejects binding a user-owned source to an outer-scope credential", () =>
    Effect.gen(function* () {
      ...
      yield* userRelay.credentialTest.registerSource(harness.scopes.userWorkspaceA.id);
      yield* setSecret(userRelay, harness.scopes.org.id, "api-token", "sk-org");
  ...
  it.effect("ignores pre-existing outer-scope bindings for an inner source", () =>
    Effect.gen(function* () {
      ...
      const listed = yield* userRelay.credentialBindings.listForSource({
        pluginId: TEST_PLUGIN_ID,
        sourceId: TEST_SOURCE_ID,
        sourceScope: harness.scopes.userWorkspaceA.id,
      });
  ...
  it.effect("source-owner cleanup removes descendant user credential bindings explicitly", () =>
    Effect.gen(function* () {
      ...
      yield* orgRelay.credentialBindings.removeForSource({
        pluginId: TEST_PLUGIN_ID,
        sourceId: TEST_SOURCE_ID,
        sourceScope: harness.scopes.org.id,
      });
  ...
  it.effect("set replaces migrated bindings by natural slot identity", () =>
    Effect.gen(function* () {
      ...
      const updated = yield* userRelay.credentialBindings.set({
        targetScope: harness.scopes.userWorkspaceA.id,
        pluginId: TEST_PLUGIN_ID,
        sourceId: TEST_SOURCE_ID,
        sourceScope: harness.scopes.org.id,
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const config = makeTestConfig({ plugins });
        const orgRelay = yield* createRelay({
          ...config,
          scopes: [scopes.org],
          plugins,
          onElicitation: "accept-all",
        });
        yield* orgRelay.credentialTest.registerSource(scopes.org.id);

        const userRelay = yield* createRelay({
          ...config,
        ...
        const secrets = yield* userRelay.secrets.list();

...
describe("usage visibility guard", () => {
  it.effect("secrets.usages returns empty when the secret id is not visible", () =>
    Effect.gen(function* () {
      ...
      const usages = yield* relay.secrets.usages(SecretId.make("other-org-secret"));
  ...
  it.effect("connections.usages returns empty when the connection id is not visible", () =>
    Effect.gen(function* () {
      ...
      const usages = yield* relay.connections.usages(ConnectionId.make("other-org-connection"));

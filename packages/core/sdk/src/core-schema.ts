...
import {
  ...
  relayUnscopedPolicyName,
  relayScopeIds,
  requireRelayScopeTarget,
...
const unscopedRelayTable = <const TColumns extends UserColumns>(
  name: string,
  columns: TColumns,
) => {
  ...
  return out.policy({
    name: relayUnscopedPolicyName,
  });
};

const scopedRelayTableBase = <const TColumns extends UserColumns>(
  name: string,
  columns: TColumns,
) => {
  const out = table(name, {
...
export const scopedRelayTable = <const TColumns extends UserColumns>(
  name: string,
  columns: TColumns,
) => {
  const out = scopedRelayTableBase(name, columns);
  return out.policy<RelayScopePolicyContext>({
    name: relayScopePolicyName,
    onRead: ({ builder, context }) =>
      builder("scope_id", "in", relayScopeIds(name, "read", context)),
    onCreate: ({ values, context }) =>
      assertRelayScopeAllowed(name, "write", values.scope_id, context),
    onUpdate: ({ builder, set, create, where, context }) => {
      const target = requireRelayScopeTarget(name, "write", where, context);
      if (set.scope_id !== undefined) {
        assertRelayScopeTargetValue(name, "write", set.scope_id, target, context);
      }
      if (create?.scope_id !== undefined) {
        assertRelayScopeTargetValue(name, "write", create.scope_id, target, context);
      }
      return builder("scope_id", "=", target.value);
    },
    onDelete: ({ builder, where, context }) => {
      const target = requireRelayScopeTarget(name, "delete", where, context);
      return builder("scope_id", "=", target.value);
    },
  });
};
...
const credentialBindingTable = (() => {
  const out = scopedRelayTableBase("credential_binding", {
  ...
  return out.policy<RelayScopePolicyContext>({
    name: relayScopePolicyName,
    onRead: ({ builder, context }) =>
      builder("scope_id", "in", relayScopeIds("credential_binding", "read", context)),
    onCreate: ({ values, context }) =>
      assertRelayScopeAllowed("credential_binding", "write", values.scope_id, context),
    onUpdate: ({ builder, set, create, where, context }) => {
    ...
    onDelete: ({ builder, where, context }) => {
      const target = requireRelayScopeTarget("credential_binding", "delete", where, context, [
        "scope_id",
        "source_scope_id",
      ]);
...
export const coreTables = defineTables({
  source: scopedRelayTable("source", {
    plugin_id: textColumn("plugin_id"),
    kind: textColumn("kind"),
  ...
  definition: scopedRelayTable("definition", {
    source_id: textColumn("source_id"),
    plugin_id: textColumn("plugin_id"),
    name: textColumn("name"),
  ...
  oauth2_session: scopedRelayTable("oauth2_session", {
    plugin_id: textColumn("plugin_id"),
  ...
  tool_policy: scopedRelayTable("tool_policy", {

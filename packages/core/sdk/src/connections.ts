...
// / `relay.connections.list`. Holds token secret ids (so a plugin can
...
export const ConnectionRef = Schema.Struct({
   ...
   *  `oauthScope` to avoid collision with the relay scope id. */
...
export const CreateConnectionInput = Schema.Struct({
  id: ConnectionId,
  /** Relay scope id that will own this connection + its backing
  ...
  expiresAt: Schema.NullOr(Schema.Number),
  /** OAuth-style scope string. Distinct from the relay scope above. */
  oauthScope: Schema.NullOr(Schema.String),
  providerState: Schema.NullOr(ConnectionProviderState),
});

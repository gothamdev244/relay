...
export interface StorageDeps<TTables extends FumaTables | undefined = FumaTables> {
  /**
   * Precedence-ordered scope stack visible to this relay. Innermost
...
// the relay from `createRelay({ onElicitation })`.
// ---------------------------------------------------------------------------
...
export interface PluginCtx<TStore = unknown> {
  ...
  readonly core: {
     ...
     *  `$ref: "#/$defs/X"` pointers; `relay.tools.schema(toolId)`
  ...
  readonly secrets: {
     ...
     *  `relay.secrets.set` on the host surface, but OAuth2 refresh
...
export interface StaticToolHandlerInput<TStore = unknown> {
   ...
   *  `createRelay({ onElicitation })` is called. */
...
export interface StaticToolDecl<TStore = unknown> {
   ...
   *  `mayElicit`. Enforced by the relay before the handler runs.
...
export interface SourceLifecycleInput<TStore = unknown> {
   ...
   * across the relay's scope stack. Plugins that own a side table
...
> {
   ...
   *  `relay.config.ts` drives both server and client) — `${packageName}/client`
   ...
   *  tables at relay startup via `collectTables`. The type flows
   ...
   *  see. The Vite plugin reads this off each `relay.config.ts` spec
   ...
   *  `relay.config.ts` flows through to the bundle automatically,
   ...
   *  `relay[plugin.id]` and is also the `self` passed to
   ...
   *  the host's typed `HttpApi<"relay", CoreGroups | PluginGroups>`
   ...
   *      requestRelay[id])` in the auth middleware
   *
  ...
  /** Invoke a dynamic tool. Called when the relay's static-handler
   ...
   *
   *  Omit if the plugin has no annotations to contribute — relay
   ...
   *  `ownerKind` / `slot` tagging the location. The relay fans out
  ...
  /** Called when `relay.sources.remove({ id, targetScope })` targets
   *  a source owned by this plugin. Plugin-side cleanup only; the
   *  relay deletes the core source/tool rows after this callback
   ...
   *  onboarding UI, `relay.sources.detect(url)` fans out to every
   ...
   *  at relay startup after `storage` and `extension` have been
   *  built. */
...
// ---------------------------------------------------------------------------
// AnyPlugin / PluginExtensions — type-level glue for the Relay surface.
// ---------------------------------------------------------------------------

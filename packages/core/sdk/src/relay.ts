...
import { assertRelayScopePolicyTable, type RelayScopePolicyContext } from "./scope-policy";
...
// ---------------------------------------------------------------------------
// Elicitation handler — set once at `createRelay({ onElicitation })`
...
//
// Required at the relay level rather than per-invoke, so the
...
// `tools.invoke(id, args, { onElicitation })` — the relay-level
...
export interface InvokeOptions {
  /** Override the relay-level handler for this single call. */
...
// ---------------------------------------------------------------------------
// Relay — public surface. Every list/invoke/schema call is a direct
...
export type Relay<TPlugins extends readonly AnyPlugin[] = readonly []> = {
  /**
   * Precedence-ordered scope stack this relay was configured with.
  ...
  readonly policies: {
    /** All policies visible across the relay's scope stack, sorted
...
export type RelayDbInput = FumaDb<any> | RelayDb;

export type RelayDbFactory = (config: {
  readonly tables: FumaTables;
}) => RelayDbInput | Effect.Effect<RelayDbInput, StorageFailure>;
...
export interface RelayConfig<TPlugins extends readonly AnyPlugin[] = readonly []> {
   ...
   */
  readonly scopes: readonly Scope[];
  readonly db?: RelayDbInput | RelayDbFactory;
...
export const collectTables = (plugins: readonly AnyPlugin[]): FumaTables => {
  ...
  for (const plugin of plugins) {
    ...
    for (const [tableKey, tableDef] of Object.entries(plugin.schema)) {
      if (merged[tableKey]) {
        // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: collectTables is a synchronous configuration API
  ...
  validateRelayScopePolicyTables(merged);

  return merged;
};

const validateRelayScopePolicyTables = (tables: FumaTables): void => {
  for (const [tableKey, tableDef] of Object.entries(tables)) {
    assertRelayScopePolicyTable(tableDef, tableKey);
  }
};

const validateRelayDbTables = (required: FumaTables, actual: FumaTables): void => {
  const missing = Object.keys(required)
    .filter((tableName) => !actual[tableName])
    .sort();
  if (missing.length === 0) return;

  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: synchronous startup validation before Relay services are built
  throw new StorageError({
    message: `Relay database is missing required table definitions: ${missing.join(", ")}`,
    cause: {
      missing,
...
const createDefaultMemoryDb = (tables: FumaTables): RelayDb => {
  const version = "1.0.0";
  ...
  const factory = fumadb({
    namespace: "relay_memory",
    schemas: [latestSchema],
  });

  // oxlint-disable-next-line relay/no-double-cast -- boundary: dynamic plugin table map is known only after collectTables()
...
const EXECUTOR_SOURCE_ID = "relay";
...
// sync from relay.jsonc can call register() on rows that already
...
export const createRelay = <const TPlugins extends readonly AnyPlugin[] = readonly []>(
  config: RelayConfig<TPlugins>,
): Effect.Effect<Relay<TPlugins>, StorageFailure> =>
  Effect.gen(function* () {
    ...
    const tables = yield* Effect.try({
      ...
      catch: (cause) => storageFailureFromUnknown("Failed to collect relay tables", cause),
    });
    ...
    yield* Effect.try({
      try: () => {
        validateRelayDbTables(tables, rootDbUntyped.internal.tables);
        validateRelayScopePolicyTables(rootDbUntyped.internal.tables);
      },
      catch: (cause) => storageFailureFromUnknown("Failed to validate relay tables", cause),
    });

    if (scopes.length === 0) {
      return yield* new StorageError({
        message: "createRelay requires a non-empty scopes array",
        cause: undefined,
      });
    }
    ...
    } satisfies RelayScopePolicyContext);
    ...
    // env or 1password don't partition their inventory by relay scope.
        ...
        if (!scopeIds.includes(input.scope)) {
          return yield* new StorageError({
            message:
              ...
              `in the relay's scope stack [${scopeIds.join(", ")}].`,
            cause: undefined,
          });
        }
    ...
    const secretsRemove = (
      ...
      Effect.gen(function* () {
        ...
        if (!scopeIds.includes(targetScope)) {
          return yield* new StorageError({
            message:
              `secret remove targetScope "${targetScope}" is not in the relay's scope stack ` +
              `[${scopeIds.join(", ")}].`,
            cause: undefined,
          });
        }
    ...
    //      relay.secrets.set(...). These carry their pinned provider
    ...
    //      never called set() through the relay.
    //
    ...
    const secretsList = (): Effect.Effect<readonly SecretRef[], StorageFailure> =>
      Effect.gen(function* () {
        ...
        // partition their inventory by relay scope.
        ...
        if (Result.isFailure(rawResult)) {
          ...
          if (err.reauthRequired) {
            return yield* new ConnectionReauthRequiredError({
              ...
              provider: ref.provider,
              // oxlint-disable-next-line relay/no-unknown-error-message -- typed: ConnectionRefreshError.message is provider-facing domain data, not an unknown caught error
              message: err["message"],
            });
          }
          return yield* err;
    ...
    const assertScopeInStack = (
      ...
      scopeIds.includes(scopeId)
        ...
        : Effect.fail(
            new StorageError({
              message: `${label} "${scopeId}" is not in the relay's scope stack ${scopeListLabel()}.`,
              cause: undefined,
            }),
          );
    ...
    for (const plugin of plugins) {
      ...
      // Plugin-owned relay tools are intentionally mounted under the
      // single `executor` namespace so source inventory is about configured
      // integrations, not plugin management surfaces:
      //   openapi.addSource -> relay.openapi.addSource
      const decls = plugin.staticSources ? plugin.staticSources(extension) : [];
      for (const source of decls) {
        const mountUnderRelay = source.kind === "relay" && source.id === plugin.id;
        const mountedSource = mountUnderRelay ? EXECUTOR_SOURCE : source;
        ...
        for (const tool of source.tools) {
          const mountedTool = mountUnderRelay
            ? {
                ...tool,
    ...
    const listSources = () =>
      Effect.gen(function* () {
        ...
        yield* Effect.annotateCurrentSpan({
          "relay.sources.static_count": staticList.length,
          "relay.sources.dynamic_count": dynamicDeduped.length,
        });
        return merged;
      }).pipe(Effect.withSpan("relay.sources.list"));
    ...
    const resolveAnnotationsFor = (rows: readonly ToolRow[]) =>
      Effect.gen(function* () {
        ...
        // `relay.tools.list.annotations` span.
    ...
    const listTools = (filter?: ToolListFilter) =>
      Effect.gen(function* () {
        ...
        const annotations =
          filter?.includeAnnotations === false
            ...
            : yield* resolveAnnotationsFor(dynamicDeduped).pipe(
                Effect.withSpan("relay.tools.list.annotations"),
              );
        ...
        yield* Effect.annotateCurrentSpan({
          "relay.tools.static_count": staticTools.size,
          "relay.tools.dynamic_count": dynamicDeduped.length,
          "relay.tools.result_count": result.length,
          "relay.tools.blocked_count": blockedCount,
        });
        return result;
      }).pipe(Effect.withSpan("relay.tools.list"));
      ...
      Effect.gen(function* () {
        const defs: Record<string, unknown> = opts.sourceId
          ? yield* loadDefinitionsForSource(opts.sourceId).pipe(
              Effect.withSpan("relay.tool.schema.load_defs"),
            )
          : {};
    ...
    const toolSchema = (toolId: string) =>
      Effect.gen(function* () {
        ...
        if (staticEntry) {
          yield* Effect.annotateCurrentSpan({
            "relay.tool.dispatch_path": "static",
            "relay.source_id": staticEntry.source.id,
            "relay.source_kind": staticEntry.source.kind,
          });
        ...
        const rows = yield* core
          ...
          .pipe(Effect.withSpan("relay.tool.resolve"));
        const row = findInnermost(rows);
        if (!row) return null;
        yield* Effect.annotateCurrentSpan({
          "relay.tool.dispatch_path": "dynamic",
          "relay.source_id": row.source_id,
          "relay.plugin_id": row.plugin_id,
        });
        return yield* buildToolSchemaView({
          toolId,
          name: row.name,
        ...
        Effect.withSpan("relay.tool.schema", {
    ...
    const invokeTool = (toolId: string, args: unknown, options?: InvokeOptions) => {
      ...
      return Effect.gen(function* () {
        const formatInvocationCauseMessage = (cause: unknown): string => {
          // oxlint-disable-next-line relay/no-instanceof-error, relay/no-unknown-error-message -- boundary: preserve public invoke error message wrapping for unknown plugin failures
        ...
        const policy = yield* resolveToolPolicyForId(toolId).pipe(
          Effect.withSpan("relay.tool.resolve_policy"),
        );
        ...
        if (staticEntry) {
          ...
          ).pipe(Effect.withSpan("relay.tool.handler"));
        }
        ...
        if (policy?.action !== "approve" && runtime.plugin.resolveAnnotations) {
          const map = yield* runtime.plugin
            ...
            .pipe(Effect.withSpan("relay.tool.resolve_annotations"));
        ...
        Effect.withSpan("relay.tool.invoke", {
          attributes: {
    ...
    const policiesList = () =>
      ...
      }).pipe(Effect.withSpan("relay.policies.list"));
    ...
    const policiesCreate = (input: CreateToolPolicyInput) =>
      ...
      }).pipe(Effect.withSpan("relay.policies.create"));
    ...
    const policiesUpdate = (input: UpdateToolPolicyInput) =>
      ...
      }).pipe(Effect.withSpan("relay.policies.update"));
    ...
    const policiesRemove = (input: RemoveToolPolicyInput) =>
      ...
      }).pipe(Effect.withSpan("relay.policies.remove"));

    const policiesResolve = (toolId: string) =>
      resolveToolPolicyForId(toolId).pipe(Effect.withSpan("relay.policies.resolve"));
          ...
          if (Effect.isEffect(out)) {
            ...
            yield* Effect.tryPromise({
              try: () => out,
                ...
                new StorageError({
                  message: "Relay database close failed",
                  cause,
                }),
            });
          }
    ...
    // Public Relay surface — storage-backed methods surface
    ...
    const toRelay = (value: unknown): Relay<TPlugins> => value as Relay<TPlugins>;
    return toRelay(Object.assign(base, extensions));
  });

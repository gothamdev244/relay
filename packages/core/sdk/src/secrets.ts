...
// plugins via `plugin.secretProviders` and registered in the relay
// at startup; there's no runtime registration.
//
// The `key` field is the provider's identifier in the secret table's
// `provider` column and in `relay.secrets.set({ provider, ... })`.
// Unique per relay.
// ---------------------------------------------------------------------------
...
export interface SecretProvider {
   ...
   *  failed, etc.) surface as `StorageFailure` — the relay treats
...
// `relay.secrets.list()`. The actual value lives in the provider
// and is only reachable via `relay.secrets.get(id)`.
// ---------------------------------------------------------------------------
...
// `relay.secrets.set(input)` takes this and writes both the
// value (to the provider) and the ref (to the `secret` table).
//
// `scope` is required — there's no default write target. Callers name
// which scope in the relay's stack should own the secret. Typical
...
export const SetSecretInput = Schema.Struct({
  id: SecretId,
  /** Scope id to own this secret. Must be one of the relay's
   *  configured scopes. */
  scope: ScopeId,
  ...
  value: Schema.String,
  /** Optional provider routing. If unset the relay picks the first

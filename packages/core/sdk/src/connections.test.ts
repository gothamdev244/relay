...
describe("connections", () => {
  it.effect("create + get + list round-trips", () =>
    Effect.gen(function* () {
      ...
      const created = yield* relay.connections.create(
      ...
      const got = yield* relay.connections.get("conn-1");
      expect(got?.id).toBe(cid("conn-1"));
      expect(got?.accessTokenSecretId).toBe(sid("conn-1.access"));

      const list = yield* relay.connections.list();
  ...
  it.effect("create fails when the provider is not registered", () =>
    Effect.gen(function* () {
      ...
      const err = yield* relay.connections
        .create(
          CreateConnectionInput.make({
            id: cid("conn-x"),
            scope: scpid("test-scope"),
  ...
  it.effect("secrets.list hides connection-owned secrets but surfaces bare ones", () =>
    Effect.gen(function* () {
      ...
      const list = yield* relay.secrets.list();
  ...
  it.effect("connections.remove cascades through providers and deletes the core row", () =>
    Effect.gen(function* () {
      ...
      // Connection row gone.
      expect(yield* relay.connections.get("conn-1")).toBeNull();
  ...
  it.effect("accessToken returns the stored value when not near expiry", () =>
    Effect.gen(function* () {
      ...
      const token = yield* relay.connections.accessToken("conn-1");
  ...
  it.effect("accessToken dedupes concurrent refreshes into a single provider call", () =>
    Effect.gen(function* () {
      ...
      const leaderFiber = yield* Effect.forkDetach(relay.connections.accessToken("conn-1"), {
      ...
      const followerFibers = yield* Effect.forEach(
        [1, 2, 3, 4],
        () =>
          Effect.forkDetach(relay.connections.accessToken("conn-1"), {
            startImmediately: true,
          }),
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const flipped = yield* relay.connections.accessToken("conn-1").pipe(Effect.flip);
  ...
  it.effect("accessToken preserves ConnectionRefreshError for non-reauth failures", () =>
    Effect.gen(function* () {
      ...
      const err = yield* relay.connections.accessToken("conn-1").pipe(Effect.flip);
  ...
  it.effect("accessToken fails with ConnectionNotFoundError for an unknown id", () =>
    Effect.gen(function* () {
      ...
      const err = yield* relay.connections.accessToken("does-not-exist").pipe(Effect.flip);
  ...
  it.effect("updateTokens writes new values but does not rotate secret ids", () =>
    Effect.gen(function* () {
      ...
      const updated = yield* relay.connections.updateTokens(
  ...
  it.effect("setIdentityLabel updates the label", () =>
    Effect.gen(function* () {
      ...
      yield* relay.connections.setIdentityLabel("conn-1", "alice@example");
      const got = yield* relay.connections.get("conn-1");
  ...
  it.effect("providers() returns every registered connection provider key", () =>
    Effect.gen(function* () {
      ...
      const keys = yield* relay.connections.providers();
...
// ---------------------------------------------------------------------------
// Multi-scope behaviour — two relays sharing an adapter, same connection
...
const makeLayeredConnRelays = () =>
  Effect.gen(function* () {
    ...
    const execOuter = yield* createRelay({
      ...config,
      scopes: [outerScope],
      plugins,
      onElicitation: "accept-all",
    });
    const execInner = yield* createRelay({
      ...config,
...
describe("connections — multi-scope behaviour", () => {
  it.effect("get picks the innermost-scope row when the same id exists at two scopes", () =>
    Effect.gen(function* () {
      const { execOuter, execInner, outerId, innerId } = yield* makeLayeredConnRelays();
      ...
      // Inner relay's list dedupes — one entry for "shared", the inner one.

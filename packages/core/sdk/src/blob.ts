...
// plugin id and bound to the relay's scope stack. Reads fall through
...
export interface PluginBlobStore {
   ...
   *  of the relay's configured scopes. */
  readonly put: (
    key: string,
    value: string,
    ...
    : Effect.fail(
        new StorageError({
          message:
            ...
            `relay's scope stack [${scopes.join(", ")}].`,
          cause: undefined,
        }),
      );
 ...
 * the relay to build the `blobs` field handed to each plugin's
 * `storage` factory.
 */

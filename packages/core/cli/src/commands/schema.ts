...
import { collectTables } from "@relay-sh/sdk/core";
...
const schemaGenerateAction = async (opts: SchemaGenerateOptions) => {
  ...
  if (!config) {
    console.error(
      "No configuration file found. Add an `relay.config.ts` file to " +
...
export const schema = new Command("schema")
  ...
  .addCommand(
    new Command("generate")
      .description("Generate an ORM schema file from the relay config")
      .option("-c, --cwd <cwd>", "the working directory", process.cwd())
      .option("--config <config>", "path to the relay config file")
      .option("--output <output>", "output file path for the generated schema")
      .option("--namespace <namespace>", "FumaDB namespace", "relay")

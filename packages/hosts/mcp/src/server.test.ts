...
import { FormElicitation, ToolId, UrlElicitation } from "@relay-sh/sdk";
import type { ExecutionEngine, ExecutionResult } from "@relay-sh/execution";
...
const makeStubEngine = <E extends Cause.YieldableError = never>(overrides: {
  ...
  getDescription: Effect.succeed(overrides.description ?? "test relay"),
});

/** Connect a real MCP Client to our relay MCP server over in-memory transports. */
const withClient = async <E extends Cause.YieldableError>(
  engine: ExecutionEngine<E>,
  capabilities: ClientCapabilities,
  fn: (client: Client) => Promise<void>,
) => {
  const mcpServer = await Effect.runPromise(createRelayMcpServer({ engine }));
  const [clientTransport, serverTransport] = InMemoryTransport.createLinkedPair();
  const client = new Client({ name: "test-client", version: "1.0.0" }, { capabilities });
  await mcpServer.connect(serverTransport);
  await client.connect(clientTransport);
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: test helper must close MCP transports after async client assertions
  try {
    await fn(client);
  } finally {
...
describe("MCP host server — client with elicitation", () => {
  ...
  it("execute tool hides defect details in MCP error results", async () => {
    const engine = makeStubEngine({
      // oxlint-disable-next-line relay/no-effect-escape-hatch, relay/no-error-constructor -- boundary: test injects a defect to verify MCP error redaction
...
describe("MCP host server — elicitation error handling", () => {
  it("elicitInput failure falls back to cancel", async () => {
    ...
    await withClient(engine, ELICITATION_CAPS, async (client) => {
      client.setRequestHandler(ElicitRequestSchema, async () => {
        // oxlint-disable-next-line relay/no-try-catch-or-throw, relay/no-error-constructor -- boundary: MCP client request handler rejects to exercise server fallback

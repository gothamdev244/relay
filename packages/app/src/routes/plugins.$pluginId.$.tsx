...
// `<RelayPluginsProvider>` (set up at the root) materialises the
// list of `ClientPluginSpec` from `virtual:relay/plugins-client`,
// and this route reads it via `useClientPlugins()` — so adding a
// plugin to `relay.config.ts` is sufficient for its pages to mount
...
function PluginRouteComponent() {
  ...
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: TanStack Router represents not-found from components by throwing notFound()

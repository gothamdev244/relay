...
import { sourcesAtom, sourcesOptimisticAtom, toolsAtom } from "@relay-sh/react/api/atoms";
import { useScope, useScopeInfo } from "@relay-sh/react/api/scope-context";
import { Button } from "@relay-sh/react/components/button";
import { SourceFavicon } from "@relay-sh/react/components/source-favicon";
import { CommandPalette } from "@relay-sh/react/components/command-palette";
import { useClientPlugins } from "@relay-sh/sdk/client";
...
function UpdateCard(props: { latestVersion: string; channel: UpdateChannel }) {
  const command = `npm i -g relay@${props.channel}`;
      ...
      {props.showBrand !== false && (
        <div className="flex h-12 shrink-0 items-center border-b border-sidebar-border px-4">
          <Link to="/" className="flex items-center gap-1.5">
            <span className="font-display text-base tracking-tight text-foreground">relay</span>
...
export function Shell() {
  ...
  useEffect(() => {
    ...
    import.meta.hot.on("relay:backend-updated", refreshBackendData);

    return () => {
      import.meta.hot?.off("relay:backend-updated", refreshBackendData);
    };

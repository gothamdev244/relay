...
const readDesktopBridge = (): DesktopBridge | null => {
  ...
  const candidate = (window as Window & { readonly relay?: DesktopBridge }).relay;
...
const buildBasicAuthHeader = (password: string): string => {
  ...
  if (typeof globalThis.btoa !== "function") {
    return `Authorization: Basic relay:${password}`;
  }
  return `Authorization: Basic ${globalThis.btoa(`relay:${password}`)}`;
};
...
export const buildMcpInstallCommand = (input: {
  ...
  const innerArgs = input.isDev ? ["bun", "run", "dev:cli", "mcp"] : ["relay", "mcp"];
  if (input.scopeDir) {
  ...
  const subtitle =
    mode === "stdio"
      ? isDev
        ...
        : "Requires the relay CLI on your PATH."
      : "Connect to relay as a remote MCP server over streamable HTTP.";

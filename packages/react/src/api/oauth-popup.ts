...
export { OAUTH_POPUP_MESSAGE_TYPE } from "@relay-sh/sdk/shared";
export type { OAuthPopupResult } from "@relay-sh/sdk/shared";
...
export const reserveOAuthPopup = (input: {
  ...
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: popup opener access can throw in browser-specific states
  try {
    popup.opener = null;
  } catch {
...
export const openOAuthPopup = <TAuth>(input: OpenOAuthPopupInput<TAuth>): (() => void) => {
  ...
  const closePopup = (popup: Window | null) => {
    if (!popup) return;
    // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: cross-origin popup state can throw and cleanup is best-effort
    try {
  ...
  // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: popup navigation can fail if the browser has invalidated the handle
  try {
  ...
  if (pollMs !== null) {
    pollHandle = setInterval(() => {
      let isClosed = false;
      // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: browser popup.closed can throw while navigating cross-origin
      try {
...
export const openOAuthSystemBrowser = <TAuth>(
  ...
  const poll = async () => {
    if (settled) return;
    // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: fetch can reject for transient network errors during polling
    try {
  ...
  void (async () => {
    // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: openExternal is host-provided IPC, no Effect runtime in this browser-only helper
    try {

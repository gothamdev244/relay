...
describe("buildRelayModule", () => {
  it("produces a valid ES module with WorkerEntrypoint import", () => {
    const module = buildRelayModule("return 42;", 5000);
    expect(module).toContain('import { WorkerEntrypoint } from "cloudflare:workers"');
    expect(module).toContain("class CodeRelay extends WorkerEntrypoint");
  });
  ...
  it("includes the timeout value", () => {
    const module = buildRelayModule("return 42;", 3000);

...
import { ExecutionToolError } from "@relay-sh/execution";
import {
  ToolDispatcher,
...
describe("makeDynamicWorkerRelay", () => {
  const loader = (env as { LOADER: WorkerLoader }).LOADER;

  it("executes simple code and returns result", async () => {
    const relay = makeDynamicWorkerRelay({ loader });
    const invoker = makeInvoker(() => null);

    const result = await Effect.runPromise(relay.execute("async () => 42", invoker));
  ...
  it("executes code that returns an object", async () => {
    ...
    const result = await Effect.runPromise(
      relay.execute('async () => ({ hello: "world" })', invoker),
    );
  ...
  it("returns error for throwing code", async () => {
    ...
    const result = await Effect.runPromise(
      relay.execute('async () => { throw new Error("boom"); }', invoker),
    );
  ...
  it("surfaces infra defects through the worker bridge as an opaque generic", async () => {
    ...
    const result = await Effect.runPromise(
      relay.execute("async () => { return await tools.secret.read({}); }", invoker),
    );
  ...
  it("preserves public ExecutionToolError messages across the worker bridge", async () => {
    ...
    const result = await Effect.runPromise(
      relay.execute("async () => await tools.search('github')", invoker),
    );
  ...
  it("collapses message-bearing object tool defects to an opaque generic", async () => {
    ...
    const result = await Effect.runPromise(
      relay.execute("async () => { return await tools.records.query({}); }", invoker),
    );
  ...
  it("returns an opaque generic when a tool result can't be serialized", async () => {
    ...
    const result = await Effect.runPromise(
      relay.execute("async () => await tools.cycles.read({})", invoker),
    );
  ...
  it("respects timeout", async () => {
    const relay = makeDynamicWorkerRelay({ loader, timeoutMs: 500 });
    const invoker = makeInvoker(() => null);

    const result = await Effect.runPromise(
      relay.execute("async () => { await new Promise(r => setTimeout(r, 5000)); }", invoker),
    );
  ...
  it("blocks fetch when globalOutbound is null", async () => {
    const relay = makeDynamicWorkerRelay({ loader, globalOutbound: null });
    const invoker = makeInvoker(() => null);

    const result = await Effect.runPromise(
      relay.execute('async () => { await fetch("https://example.com"); }', invoker),
    );
  ...
  it.effect("tools proxy supports deep paths (>2 segments)", () =>
    Effect.gen(function* () {
      ...
      const result = yield* relay.execute("async () => tools.a.b.c.d.e({})", invoker);

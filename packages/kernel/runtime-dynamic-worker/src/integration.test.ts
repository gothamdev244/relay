...
// makeRelayToolInvoker → openApiPlugin → recording HttpClient.
//
...
import { makeRelayToolInvoker } from "@relay-sh/execution";
import { openApiPlugin } from "@relay-sh/plugin-openapi";

import { makeDynamicWorkerRelay } from "./relay";
...
const buildSandboxBridge = (spec: string, namespace: string, baseUrl = "https://upstream.test") =>
  Effect.acquireRelease(
    Effect.gen(function* () {
      ...
      yield* relay.openapi.addSpec({ spec, scope: TEST_SCOPE, namespace, baseUrl });
      const invoker = makeRelayToolInvoker(relay, { invokeOptions: autoApprove });
      return { relay, invoker, captured: recording.captured, sql };
    }),
    ({ relay, sql }) =>
      relay.close().pipe(
        Effect.ignore,
...
describe("sandbox → openApiPlugin integration", () => {
  it.effect("multipart with Blob: file part contains the original bytes", () =>
    Effect.gen(function* () {
      ...
      const sandbox = makeDynamicWorkerRelay({ loader });

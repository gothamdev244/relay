...
export type QuickJsRelayOptions = {
  timeoutMs?: number;
...
const EXECUTION_FILENAME = "relay-quickjs-runtime.js";
...
//     passed through `makeRelayToolInvoker`'s catchCause) or an
...
const sandboxDefectMessage = (cause: unknown): string => {
  if (cause !== null && typeof cause === "object") {
    ...
    // oxlint-disable-next-line relay/no-manual-tag-check, relay/no-unknown-error-message -- boundary: QuickJS reject path receives an already-rendered ExecutionToolError; we pass its in-band public message through and otherwise emit the opaque generic
  ...
  context.newFunction("__executor_invokeTool", (pathHandle, argsHandle) => {
    ...
    void runPromise(toolInvoker.invoke({ path, args })).then(
      ...
      (cause) => {
        ...
        try {
          console.error("[relay:quickjs] tool dispatch defect", { path, cause });
        } catch {
...
const runInQuickJs = (
  ...
  Effect.gen(function* () {
    ...
    Effect.withSpan("relay.code.exec.quickjs", {
      attributes: { "relay.runtime": "quickjs" },
    }),
  );

export const makeQuickJsRelay = (
  options: QuickJsRelayOptions = {},
): CodeRelay<QuickJsExecutionError> => ({

...
import { makeQuickJsRelay } from "./index";
...
const relay = makeQuickJsRelay({ timeoutMs: 5_000 });

describe("quickjs executor", () => {
  it.effect("runs plain code", () =>
    Effect.gen(function* () {
      const result = yield* relay.execute(`return 1 + 2`, makeTestInvoker({}));
  ...
  it.effect("internal defects reach the sandbox as an opaque generic only", () =>
    Effect.gen(function* () {
      ...
      // because it bypasses makeRelayToolInvoker).
  ...
  it.effect("applies a memory limit by default", () =>
    Effect.gen(function* () {
      const defaultRelay = makeQuickJsRelay({ timeoutMs: 5_000 });

      const result = yield* defaultRelay.execute(
        `

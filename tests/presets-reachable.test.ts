...
import { createRelay } from "../packages/core/sdk/src/index";
        ...
        Effect.gen(function* () {
          const result = yield* introspect(preset.url).pipe(
            ...
            Effect.catchTag("GraphqlIntrospectionError", (err) =>
              Effect.succeed({
                ok: false as const,
                // oxlint-disable-next-line relay/no-unknown-error-message -- boundary: catchTag narrows to GraphqlIntrospectionError whose public contract includes message
                message: err.message,
              }),
            ),
          );
...
// ---------------------------------------------------------------------------
// Detection — full relay pipeline, only for presets that don't need auth
// ---------------------------------------------------------------------------
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          const results = yield* relay.sources.detect(preset.url);
        ...
        Effect.gen(function* () {
          const response = yield* Effect.tryPromise(() =>
            fetch(preset.icon!, {
              method: "GET",
              signal: AbortSignal.timeout(10_000),
              headers: { "User-Agent": "relay-preset-test" },
              redirect: "follow",
            }),
          );

...
describe("CLI tooling helpers", () => {
  ...
  it.effect("rejects non-object JSON input", () =>
    Effect.gen(function* () {
      ...
      // oxlint-disable-next-line relay/no-unknown-error-message -- boundary: helper contract returns a native Error for CLI input parsing
  ...
  it("builds search and sources code snippets", () => {
    ...
    expect(sourcesCode).toBe(
      'return await tools.relay.sources.list({"limit":20,"query":"google"});',
    );
  });

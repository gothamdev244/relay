...
const currentPlatformPackage = `relay-${platformName}-${archName}`;
const currentRuntimeBinaryName = process.platform === "win32" ? "relay.exe" : "relay";
...
describe("release bootstrap smoke", () => {
  it("wrapper resolves the platform binary from optionalDependencies and stays runnable", async () => {
    ...
    const wrapperDir = join(distDir, "relay");
    const platformDir = join(distDir, currentPlatformPackage);

    // Simulate the install layout npm/bun produces:
    //   <root>/relay/                <- wrapper (bin, postinstall, package.json)
    //   <root>/relay/node_modules/relay-<plat>-<arch>/  <- platform package
    const tempRoot = await mkdtemp(join(tmpdir(), "relay-optdeps-bootstrap-"));
    const installedWrapperDir = join(tempRoot, "relay");
    ...
    // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: release smoke test must clean temp install files after process checks
    try {
      const firstRun = await runCommand(
        process.execPath,
        [join(installedWrapperDir, "bin", "relay"), "--help"],
        installedWrapperDir,
      );
      ...
      const webProcess = spawn(
        process.execPath,
        [join(installedWrapperDir, "bin", "relay"), "web", "--port", String(webPort)],
        {
      ...
      // oxlint-disable-next-line relay/no-try-catch-or-throw -- boundary: release smoke test must stop the spawned web process
      try {

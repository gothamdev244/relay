...
- Prior convention in this repo uses **`patch`** bumps for feature-heavy releases (see `.changeset/relay-1.4.6-beta.md` for precedent). Don't push back on patch unless there are genuine SemVer-breaking API changes to a library consumer surface.
...
- Normal release/patch PRs must add a `.changeset/*.md` file with frontmatter like `"relay": patch`. Do **not** directly bump `apps/cli/package.json` or `bun.lock` in a feature/fix PR.
- Only the Changesets-generated `Version Packages` PR should move `apps/cli/package.json`. If a normal PR directly changes that version, merging it to `main` can make `.github/workflows/release.yml` tag the commit and dispatch `publish-relay-package.yml`, causing an immediate CLI publish.
- `@relay-sh/*` library packages have their own publish path.
...
- If your PR adds a `.changeset/*.md` for the `relay` package, also edit `apps/cli/release-notes/next.md` for the user-facing story. They have different audiences.
...
- Frontmatter is `"relay": patch` (or `minor`/`major` if owner says so).
...
- Users install: `npm i -g relay@beta`.
...
bun run release:publish:packages:dry-run   # pack @relay-sh/* without publishing
...
2. Merging that PR:
   - Publishes every `@relay-sh/*` library that's not yet on npm (via `scripts/publish-packages.ts`).
   - If `apps/cli/package.json` bumped, tags the commit and dispatches `publish-relay-package.yml`, which runs `release:check`, does a full dry-run build, publishes the CLI to npm, and creates/updates the GitHub Release with binary assets.
